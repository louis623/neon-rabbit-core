...
- Control Center operators can mark scheduled waitlist conversations as `conversation_complete` after the manual setup call. This only records waitlist state; transcript import, profile drafting, Start Work decisions, calendar, SMS, email, Stripe, SignWell, and live queue work remain separate operator-led steps.
...
- Production deploy `https://sparkle-suite-172smwbr3-louis-2849s-projects.vercel.app` from commit `e7826cf` is aliased to `https://www.yoursparklesuite.com` and passed public-domain route smoke for demo auth, Nic-Nac shell, Stripe test checkout session, and Stripe test portal session; report: `.local/launch-smoke-results/launch-preview-2026-05-19T21-16-46-317Z.json`.
- Vercel env now has `SIGNWELL_API_KEY`, `SIGNWELL_API_BASE_URL`, and `SIGNWELL_TEMPLATE_ID` in Development and Preview for `codex/sparkle-cross-phase-hardening`; `SIGNWELL_TEMPLATE_RECIPIENT_PLACEHOLDER=Client` is set in Development, branch Preview, and Production.
...
flowchart LR
  ...
  G -. "PARTIAL: SignWell sandbox provider passed; one paid Nic-Nac request passed; live SMS/SignWell/Stripe still gated" .-> G1["Later"]
```
...
- SignWell sandbox provider contact passed on 2026-05-19 after a read-only template fetch showed the template recipient placeholder is `Client`. The successful provider smoke created a test-mode draft document with `send_email=false`, `draft=true`, `test_mode=true`, and no live agreement email.
- Production was redeployed from clean commit `e7826cf` on 2026-05-19 after SignWell draft-mode hardening and the `Client` placeholder env update. Public-domain route smoke passed afterward.
