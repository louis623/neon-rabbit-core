...
**Version:** 1.8 | **Created:** April 5, 2026 | **Last Updated:** April 17, 2026 | **Status:** LOCKED
...
| File | Status |
|---|---|
| `L1_NR_Document_System_SOP_v1.8.md` | ✅ Live (this file) |
| `L1_NR_Plugins_Skills_Standing_Rules_v3.9.md` | ✅ Live |
...
### Layer 6 — Supabase Structured Data (neon-rabbit-core) — EXPANDED April 16-17, 2026
...
**Core tables (Memory Library ship, April 16-17, 2026):**

| Table | Purpose | Writers |
|---|---|---|
| `thoughts` + embeddings | Open Brain — decisions, session logs, ideas | Claude Chat via open-brain-mcp, Rabbit Hole app |
...
|---|---|---|
| `open-brain-mcp` | Claude's working memory (thoughts, decisions, session logs) | Read + write via `capture_thought`, `search_thoughts`, `list_thoughts`, `thought_stats` |
| `nr-hq-mcp` | Build tracking, governance, clients, audit | 18 tools (6 reads + 12 writes); gated by `MCP_ACCESS_KEY` |

**nr-hq-mcp tool surface (after Task 4 Part A ship, April 17, 2026):**

Reads (6): `get_phases`, `get_tasks`, `get_gates`, `get_action_cards`, `get_build_summary`, `get_recent_audit_log`

Writes (12): `update_task_status`, `update_phase_status`, `update_gate_status`, `update_action_cards`, `create_open_item`, `update_open_item`, `resolve_open_item`, `get_open_items`, `create_client`, `update_client`, `get_clients`, `get_client`

**CRITICAL CONVENTION — actor param:** The 4 status-change write tools (`update_task_status`, `update_phase_status`, `update_gate_status`, `update_action_cards`) accept an optional `actor` param (`'chat'` | `'claude_code'`, default `'claude_code'`). **Claude Chat MUST pass `actor='chat'` on every call** so audit rows label changes correctly. Not enforced server-side — behavioral rule.

**Trust boundary for audit data:** Audit rows (rows with `entry_kind='audit'` in `build_action_log`) are NOT exposed via anon Supabase access. RLS policy restricts anon SELECT to `entry_kind='card_snapshot'` only. Audit rows are readable ONLY via the `get_recent_audit_log` MCP tool, which uses the service-role path and is gated by `MCP_ACCESS_KEY`.

**Rules:**
- pgvector enabled, RLS on all tables
- No binary files — rows and structured data only
- After any Edge Function deployment (including tool additions or signature changes), Louis must reload the NR HQ connector in Claude.ai Settings → Connectors → Disconnect → Reconnect so fresh chats see the updated tool schemas
...
## How the Layers Connect (Data Flow — UPDATED April 17, 2026)

```
Louis talks to Claude Chat
    → Decisions captured to Open Brain (Layer 3) via open-brain-mcp
...
**Key shift from v1.7:** Session close no longer includes a "generate Build Tracker update prompt" step. Claude Chat writes to Supabase directly during the session via MCP tools. Build Tracker state is live at all times.
...
1. Claude generates a brand new Markdown file with an incremented version number in the filename (e.g. `_v1.8.md`)
...
Every session must begin with these four pulls before any work starts:
...
Pull #4 replaces uploading Markdown status files for build status. Claude gets live build reality from Supabase in one call. Open Brain pulls (1–3) still carry decisions, context, lessons learned, and concerns that structured data cannot capture.
...
After completing all four pulls, confirm status back to Louis before touching any task:

> "✅ Session open complete. Here's where we stand: [summary]. Build status: [from MCP]. Active tasks: [list]. Next action: [specific first step]. Anything missing or wrong?"

---

## Session Close Protocol (SIMPLIFIED April 17, 2026)
...
Common triggers: new standing rules added, session protocol changes, document system changes, architecture decisions that affect spec files, completed build phases that update project specs.

**Note (v1.8):** Build Tracker status updates are NOT part of session close file generation. Those update live via MCP during the session.
...
- What to pull from Open Brain (specific search terms)
...
Examples:
L1_NR_Document_System_SOP_v1.8.md
L1_NR_Plugins_Skills_Standing_Rules_v3.9.md
