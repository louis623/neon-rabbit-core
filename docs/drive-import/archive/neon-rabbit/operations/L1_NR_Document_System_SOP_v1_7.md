...
**Version:** 1.7 | **Created:** April 5, 2026 | **Last Updated:** April 14, 2026 | **Status:** LOCKED
...
| File | Status |
|---|---|
| `L1_NR_Document_System_SOP_v1.7.md` | ✅ Live |
| `L1_NR_Plugins_Skills_Standing_Rules_v3.8.md` | ✅ Live |
...
**What lives here:** All structured operational data — client records, project status, financials, Open Brain embeddings. Also serves as the backend for MCP read access from Claude Chat.

**Project:** `neon-rabbit-core`, us-east-1, ref `bqhzfkgkjyuhlsozpylf`

**MCP Endpoints:**
- `open-brain-mcp` — Claude's working memory (thoughts, decisions, session logs). Read/write.
- `nr-hq-mcp` — Build tracking and operational data (phases, tasks, gates, action cards). Read-only. Added April 14, 2026.

Both endpoints are Edge Functions deployed to neon-rabbit-core. Source code lives at `sparkle-suite/supabase/functions/`. Auth: service_role + MCP_ACCESS_KEY gate.
...
Claude Chat reads build status
    → NR HQ MCP (Layer 6) → construction_phases, tasks, gates, action_log
...
Examples:
L1_NR_Document_System_SOP_v1.7.md
L1_NR_Plugins_Skills_Standing_Rules_v3.8.md
...
| Build status (live, queryable by Claude) | 6 | Supabase via NR HQ MCP (`get_build_summary`, `get_phases`, `get_tasks`, `get_gates`, `get_action_cards`) |
