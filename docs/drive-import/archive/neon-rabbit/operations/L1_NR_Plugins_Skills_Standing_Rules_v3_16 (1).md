...
4. **Guardian Agents** (captured April 21, 2026) — NR-wide defensive/health/maintenance agent layer (Thumper Guardian first). Sparkle Suite Phase 1 ships with structural hooks (thumper_incidents table, /api/thumper/health endpoint, tool execution telemetry) so retrofit is cheap post-launch.
5. **Enforcer Agents** (captured April 21, 2026) — NR-wide offensive/security/law-and-order agent layer (complementary to Guardians). Sparkle Suite Phase 1 ships with structural hooks (auth_events, tool-call audit log, trade action audit log, SMS/email blast audit log). Full Enforcer build post-launch.
