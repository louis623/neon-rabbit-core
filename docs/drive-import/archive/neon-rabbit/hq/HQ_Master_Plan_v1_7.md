...
**Version:** 1.7 | **Created:** April 6, 2026 | **Last Updated:** April 14, 2026 | **Status:** ALL SECTIONS COMPLETE — DASHBOARD RESTRUCTURE IN PROGRESS (Tasks 1–4 complete, Tasks 5–7 pending)
**Supersedes:** HQ_Master_Plan_v1.6.md (retired), all prior versions
...
| Prompts | Prompt template library (NOT YET BUILT — needs content session) | TBD |
| Tools | CLI cheat sheets, tool references | CLI |

**Changes from Phase 2A baseline (April 14, 2026):**
- Maintenance tab → Operations sub-tab (commit d01bdfa)
- PA tab → Build Tracker sub-tab as "VA Compensation" (commit 35ca37f)
- Client Directory added as Operations sub-tab (commit 9570b69)
- Tools tab added with combined CLI reference (commits 4784680 + b5bc501)
- Prompts tab planned (needs content session before build)
- Top nav reduced from 10 to 9 tabs (cleaner)
...
|---|---|---|
| **2A** | Schema + Auth + Manual Entry + Validation | April 4, 2026 |
| **2B** | Stripe + Plaid APIs, daily cron, briefs, OB pipeline | April 13, 2026 |
| **Build Tracker tab** | Phase progress, task checklists, test gate tracking for SS build | April 13, 2026 — commits 2d588d4 + 33fac6e. 3 Supabase tables, 5 UI components, ~95 tasks seeded. |
| **Visual Redesign** | Full NR branding, NRCard hover glow, SVG progress rings, flow map, wave bars, action cards, dark mode only | April 13, 2026 — commit 3db7a10. 37 files changed. |
...
| **Dashboard Restructure (Tasks 1–4)** | Tools tab, Maintenance→Operations, PA→Build Tracker, Client Directory | April 14, 2026 — commits 4784680, b5bc501, d01bdfa, 35ca37f, 9570b69. See HQ_Dashboard_Restructure_Plan_v1.1 for details. |
...
|---|---|---|
| **Claude Chat Supabase Read Access** | Extend Open Brain MCP or new Edge Function for Claude Chat to query construction tables + build_action_log directly | Opus architecture session (next session) |
| **Dashboard Restructure (Tasks 5–7)** | Prompts tab (needs content session), VA Compensation workflow redesign, Client Directory Supabase wiring + auto-update | See HQ_Dashboard_Restructure_Plan_v1.1 |
...
8. ⏳ Claude Chat Supabase read access (Opus architecture) — next session
9. ⏳ Dashboard restructure Tasks 5–7 (Prompts content session, VA workflow redesign, Client Directory wiring)
...
| Apr 14 | Top nav cleaned: 10 tabs → 9 tabs. Prompts tab planned but not yet built (needs content session) |
...
| Apr 14 | Claude Chat Supabase read access (Option D) — approved, Opus architecture session next |
| Apr 14 | Gemini worth testing for frontend/UX builds — future comparison against Claude Code |
| Apr 14 | Louis wants to continue HQ restructure before returning to Sparkle Suite builds |
| Apr 14 | HQ_Dashboard_Restructure_Plan_v1.1 created — tracks all restructure tasks |
