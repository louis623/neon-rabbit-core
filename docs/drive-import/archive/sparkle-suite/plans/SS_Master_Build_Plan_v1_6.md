...
**Version:** 1.6 | **Created:** April 9, 2026 | **Last Updated:** April 11, 2026 | **Status:** ACTIVE — Phase 0 IN PROGRESS. Tasks 0.1 (schema) and 0.1b (service layer) COMPLETE. Next: write Claude Code prompts for Tasks 0.2–0.6 (most can run overnight). All gaps resolved except Gap 4 (DUCLUS test — does not block).
...
| 0.2 | Create all tables + RLS policies in Supabase | 🔧 Medium | 🌙 Yes | Claude Code. Single session. Reference BOTH SS_Supabase_Schema_v1_0.md AND SS_Service_Layer_Spec_v1_0.md. Every table gets RLS from day one — no exceptions. Admin role for Louis sees across all reps. ALSO CREATE: (1) partial unique index on trade_requests for one-request-per-piece rule, (2) GIN index on jewelry_designs for full-text search, (3) three Postgres RPC functions (rpc_submit_trade_request, rpc_approve_trade, rpc_reject_trade). See service layer spec for exact SQL. |
