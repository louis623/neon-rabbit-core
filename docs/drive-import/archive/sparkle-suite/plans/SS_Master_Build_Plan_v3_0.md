...
**Version:** 3.0 | **Created:** April 9, 2026 | **Last Updated:** April 18, 2026 | **Status:** ACTIVE
...
- SS_Supabase_Schema_v1_0.md — 16-table schema specification (source of truth for Phase 0 schema)
- SS_Service_Layer_Spec_v1_0.md — Shared service layer: 12 functions, 3 Postgres RPCs, error handling, security (source of truth for Phases 1.5, 3.x, 6.x)
...
**Dual interface principle:** Every operation Thumper can perform has a dashboard UI equivalent. Both call the same shared service layer. Neither is read-only.
...
| 0.4 | Stripe integration — subscription billing | 🔧 Medium | 🌙 Yes | Stripe client library (lazy init, Zod config validation), customer creation (idempotent getOrCreate), Checkout Session API, webhook handler with idempotency via stripe_events table, portal session, subscription status, pro-rata refund with state machine (refund_operations table). All non-webhook routes derive repId from cookies. |
...
- **Platform pricing** (monthly/quarterly/annual amounts) — Louis decision session needed. Informed by TAM data (20K–50K reps, 5–10% penetration target) and Thumper API cost modeling.
