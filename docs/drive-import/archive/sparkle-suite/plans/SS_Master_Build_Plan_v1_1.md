...
**Version:** 1.1 | **Created:** April 9, 2026 | **Status:** ACTIVE — Pre-build planning complete. Awaiting Gap 20 + Gap 22 resolution before prototype build begins. Louis reviewing plan and building out NR HQ in the meantime.
...
```
Gap 20 (Lindsey) → Gap 22 (tool schemas)
         ↓
...
|---|------|-------|--------|--------|
| 1 | Gap 20 — BP item number confirmation | Louis → Lindsey | ⏳ OPEN — asking tomorrow | Determines whether dedup is simple (item number match) or complex (fuzzy matching). Affects Gap 22 tool parameters. BLOCKING prototype build. |
| 2 | Gap 22 — Thumper trade board tool schemas | Claude (Opus session) | ⏳ PARKED until Gap 20 answered | Specific tool definitions (parameters, return values, error handling) for all trade board operations. Must be designed before Claude Code touches the prototype. ~30–45 min Opus session. |
...
| 4 | Formal build sequence session | Claude + Louis | ⏳ THIS DOCUMENT | This master plan IS the build sequence session. Completing this plan resolves this item. |

**After Gap 20 is answered:** Immediately run the Gap 22 Opus session (same day if possible). Once Gap 22 is complete, Phase 0 can begin.
...
| 3.8 | Dedup matching logic | 🔧 Medium | Depends on Gap 20 | If BP has item numbers: simple exact match. If not: collection + type + material + MSRP fallback with fuzzy search. Rep always confirms — not fully automated. |
...
**What this builds:** The full rep onboarding flow from first contact to live site. Plus the rep-facing landing page at yoursparklesuite.com.
...
**Estimated time:** 🏢 Multi-day — 4–6 Claude Code sessions over 3–4 days
...
| 8.4 | Pre-meeting intel agent | 🔧 Medium | 🌙 Yes | Before discovery call, agent researches rep's social presence, streaming history, team size. Generates intel brief for Louis. |
| 8.5 | Google Meet + Gemini transcript hook | 🔧 Medium | Needs research | Gemini transcribes discovery call. Transcript feeds post-meeting agent. Integration approach TBD. |
| 8.6 | Post-meeting agent | 🔧 Medium | 🌙 Yes | Processes transcript. Generates rep profile draft. Populates branding preferences. Prepares agreement. |
| 8.7 | Gate 1 — SignWell agreement with clickwrap audit trail | 🔧 Medium | 🌙 Yes | IP address, timestamp, document hash captured. Must complete before build starts. |
| 8.8 | Gate 2 — Start work fee via Stripe | ⚡ Quick | 🌙 Yes | Stripe payment link or checkout. Webhook fires → unlocks build. Start fee = "earned upon payment," non-refundable. |
| 8.9 | Agentic site build trigger | 🏗 Large | 🌙 Yes — primary overnight candidate | After Gates 1+2 clear: agent spins up rep's site automatically. Populates template from branding interview data. Creates Supabase profile. Generates QR code. |
| 8.10 | Gate 3 — Launch fee via Stripe | ⚡ Quick | 🌙 Yes | Holds deployment until launch fee clears. Site goes live only after payment confirmed. |
| 8.11 | QR code auto-generation | ⚡ Quick | 🌙 Yes | Generated automatically during site spin-up. Links to rep's custom domain. Ready to download immediately. |
| 8.12 | Branding menu | 🔧 Medium | Needs design session | Menu of options rep selects from during onboarding. Template choice, color preferences, hero image direction. Details TBD. |

### Test Gate 8

- [ ] Landing page loads at yoursparklesuite.com with intake form and Thumper
- [ ] Waitlist captures data correctly
- [ ] Full pipeline test: intake → scheduling → meeting → agreement → payment → site build
...
- [ ] Agentic build produces a working site from branding data (test with dummy rep)
...
|-------------|-------------------|-----------|------------------------|
| Gap 20 — BP item numbers | Phase 3.8 (dedup matching) | Lindsey — tomorrow | If YES (item numbers exist): dedup is simple exact match, Phase 3.8 is ⚡ Quick. If NO: multi-attribute fuzzy matching, Phase 3.8 is 🔧 Medium. |
| Gap 22 — Tool schemas | Phase 1.5 (Thumper trade tools) | Opus session after Gap 20 | Defines exact parameters for all trade board tools. Shapes Phase 1.5 and Phase 3.5. |
