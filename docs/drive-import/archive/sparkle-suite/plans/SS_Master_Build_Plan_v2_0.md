...
🔄 UPDATE TRIGGER: Any phase completion, scope change, dependency shift, test gate result, legal finding, or parking lot item moved to active

**Version:** 2.0 | **Created:** April 9, 2026 | **Last Updated:** April 15, 2026 | **Status:** ACTIVE — Phase 0 COMPLETE. All 7 tasks verified and committed. Test Gate 0 ready to run. Ready for Phase 1 + Phase 2 parallel sprint. All gaps resolved except Gap 4 (DUCLUS lightbox arrived April 12, testing in progress — does not block).

**v2.0 CHANGE:** Absorbed content from SS_KB_OpenItems_v2.4.md and SS_KB_Legal (Open Brain). Those documents are now RETIRED. This master plan is the single source of truth for all Sparkle Suite build planning, legal foundations, deferred items, and future ideas.
...
**Adversarial review standard for money-moving code (established Session #27):** Three rounds of Codex adversarial review before Claude Code builds any feature that touches money (payments, wallet operations, billing). Each round finds real blockers the previous round missed.
...
**KEY BUSINESS RULE (Session #23):** Trade request model is ONE REQUEST PER PIECE AT A TIME. First-come first-served. Piece disappears completely from board when pending. Reappears if rejected. This supersedes any earlier Gap 22 captures about auto-cancelling other pending requests on approval.

**DEVELOPMENT APPROACH (Session #24):** All testing runs on the permanent test rep sandbox account (testrep@neonrabbit.net) — NOT Lindsey's live site. Louis acts as the rep during testing. Lindsey validates by reviewing the test rep site. Real migration via clean onboarding flow after validation passes.
...
All legal research is complete (6 sprints, Sessions #17–18). No language has been drafted — an attorney session is needed before launch. This section captures the key findings and build requirements that came from legal research so they don't get lost.
...
| L6 | Cancellation/refund policy | Universal pro-rata refund for unused months, cancel at end of current month, cancel anytime. Forever tier evaluated and ELIMINATED — creates permanent liability, variable AI costs, inactivity risk. Visa/Mastercard 7-day reminder rule for subscriptions billing every 6+ months. Visa CE 3.0 chargeback defense requires logging IP addresses, device IDs, usage timestamps from day one. |
...
| Visa 7-day pre-billing reminder for annual plans | L6 | Phase 5 (email automation) |
...
8. **Cancellation policy language review** (L6) — Pro-rata refund policy, annual renewal notice language, billing descriptor review.
...
| 2 | Attorney session (8 agenda items — see Legal Foundation section) | Louis | Agenda ready. Schedule when revenue supports. | YES — need service agreement + disclaimers before taking money |
...
These gaps were identified during the gap analysis arc (Sessions #11–21) but explicitly deferred because they don't block the current build. They become relevant at the phases noted below.

| Gap | Topic | Deferred Until | Why Deferred |
|-----|-------|---------------|--------------|
| 10 | BP community trade board examples | After Phase 3 (rep trade board built) | Need working product to compare against. Revisit to see how BP's own trade system differs and where SS adds value. |
| 12 | Lindsey revenue data | Pre-launch pricing session | Gap 11 (TAM) resolved — 20K–50K active BP reps, revenue model validated at 5–10% penetration. Revisit Lindsey's actual numbers when pricing decisions happen. |
| 13 | Business card pipeline | Post-launch | Research sprint needed for vendor, design, pricing. Not blocking anything — add-on product. Already on Pre-Launch Checklist as item #10. |
| 14 | Thumper capability boundaries | During Phase 1.2 (system prompt) | Test in context of real build. Will surface naturally during system prompt design session — what can Thumper do, what should it refuse, where are the edges? |
| 19 | Infrastructure cost modeling | Pre-launch | Build first, model real numbers. Includes Vercel, Supabase, Claude API, Telnyx, Resend, Photoroom, PostHog. Estimate once real usage patterns are visible from prototype testing. |
...
- **Platform pricing** (monthly/quarterly/annual amounts) — Louis decision session needed. Informed by Gap 11 TAM data (20K–50K reps, 5–10% penetration target) and Gap 5 cost modeling.
- **Start work fee and launch fee amounts** — Louis decision session needed. Affects Gates 2 and 3 in Phase 8.
- **Photography kit pricing tiers** — Lightbox-only vs. lightbox+camera impact on start fee. Needs DUCLUS test results first (Gap 4).
...
## Resolved Gap Analysis Record

All 13 knowledge gaps from the gap analysis arc (Sessions #11–21) are resolved. This table is the permanent record — no separate document needed.
...
| 5 | Chatbot API cost modeling | Haiku 4.5 default, Sonnet 4.6 escalation. ~$1.80–$8.50/rep/mo at heavy usage. Prompt caching from day one. Scope lock to allowed domains. | #13 |
...
| 15 | SEO/GEO multi-tenant | Research complete. Localized wrappers, schema upgrades, ISR/SSG rendering, Markdown for Agents. No architecture-breaking findings. | #16 |
...
| 20 | BP item number confirmation | BP items have CONSISTENT, STRUCTURED item numbers: two-letter type prefix + 5 digits. Prefixes: RG, NK, ER, ST, BR. Dedup is simple exact match. Collection NOT on label — Thumper asks rep when missing. | #20 |
...
| 22 | Thumper trade board tool schemas | 10 tools fully designed. Dual interface principle locked. Shared service layer required. One-request-per-piece business rule. | #21 |
...
|-----|-------|--------|-----------|
| 4 | AI photo enhancement vendor + photography kit | Photoroom primary, Claid backup. Photography kit: lightbox standard, camera flexible (three tiers). | DUCLUS lightbox arrived April 12. Louis testing with real BP jewelry. If pass: standardize, test USB webcam options. If fail: find alternative. Does NOT block prototype. |
...
From Gap 5 research (Session #13). Confirmed pricing as of April 2026:
...
- Minor (v2.0 → v2.1): Task added/removed, time estimate adjusted, dependency shifted
- Major (v2.x → v3.0): Phase restructured, critical path changed, scope significantly altered
...
*This plan is the single source of truth for the entire Sparkle Suite platform build. It covers build phases, legal foundations, deferred research, future ideas, and all resolved gap analysis decisions. SS_KB_OpenItems and SS_KB_Legal are RETIRED — their content lives here now. Update this plan as phases complete. Do not update for ideas still in brainstorming — those go to Open Brain first.*
