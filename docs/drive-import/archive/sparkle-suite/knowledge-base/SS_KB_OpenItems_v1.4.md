...
**Version:** 1.4 | **Derived from:** SS_KB_OpenItems_v1.3 | **Last Updated:** April 9, 2026
**Status:** Gap Analysis Session #14 complete. Gap 4 partially resolved (vendor selected, QA in principle). Gap 6 partially resolved (voice deferred, Thumper agent architecture open). Gaps 15, 16 still open. Legal L1–L6 prompts not yet written.

**COMPANION MODULES:**
- SS_KB_Core_v1.4.md — Architecture, strategy, business model, decisions
...
**Resolved Sessions #9–14:** 11 closed + 4 deferred + 2 partially resolved
**Remaining research sprints:** 2 fully open + 2 partially resolved + 6 legal sprints
...
10. ~~F1 + Gap 6 Chatbot quality risk / voice~~ → PARTIALLY RESOLVED. Voice deferred to post-launch. Wispr Flow recommended to reps. Thumper agent architecture OPEN — dedicated Opus session needed.
...
| 6 | Voice interface / Thumper agent | VOICE DEFERRED — text-first at launch. Wispr Flow recommended to reps. | THUMPER AGENT ARCHITECTURE not designed. How Thumper executes actions, manages tools, connects to Supabase, maintains context = open. Dedicated Opus session required. |
...
## Thumper Agent Architecture — CRITICAL OPEN ITEM (Session #14)

The voice layer of Gap 6 is resolved (text-first, Wispr Flow recommendation). The agent execution layer is NOT resolved and has not been researched or designed.

**What's unknown:**
- What framework or orchestration layer manages Thumper's actions (Vapi? Custom? Vercel AI SDK alone?)
- How Thumper executes actions against Supabase (tool-calling architecture — what tools exist, how are they defined?)
- How Thumper maintains conversation context across a session
- How Thumper handles multi-step workflows (e.g., "add a trade listing" requires photo upload, description, MSRP, category — how does the conversation flow?)
- How agentic actions are authenticated and scoped per rep (RLS passthrough?)
- Error handling and graceful failure when an action can't be completed
- How the Vercel AI SDK, Claude API tool-calling, and Supabase interact in practice

**Recommended approach:** Dedicated design session with Claude Opus (technical complexity warrants Opus). Session should produce a locked Thumper agent architecture spec before any Claude Code work begins on Thumper.

**This is the most critical open item before the build sequence can be finalized.**
...
## Key Decisions Made Sessions #11–14

### Onboarding Workflow (B3) — Full Pipeline
Seven-step agentic pipeline from landing page intake through agentic build. See SS_KB_Core_v1.4 for full detail.
...
- Agent architecture = OPEN. Dedicated Opus session required.
...
- Launch fee amount
- Custom work pricing
- Thumper agent architecture (Gap 6 open — Opus session required)
