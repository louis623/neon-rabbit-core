...
**Version:** 1.6 | **Derived from:** SS_KB_Core_v1.5 | **Last Updated:** April 9, 2026
**Status:** WORKING — updated with Session #16 decisions (Gap 15 SEO/GEO research complete, Gap 16 jewelry database three-table data model designed, live show trade flow decided, MSRP/rarity decisions, filtering requirements, Lindsey prototype scope with Thumper)

**COMPANION MODULES:**
- SS_KB_SiteSpec_v1.0.md — Full site template spec (all 4 pages + design system)
- SS_KB_OpenItems_v1.6.md — Open questions, research sprint, gap analysis, parking lot
...
**Self-service tech support (Session #16):** Thumper must be capable of walking reps through ALL routine platform setup — browser extension installation, dashboard setup, trade board configuration, show prep, general how-to guidance. Louis cannot personally help 100 reps. Thumper is the front line for everything except escalations and relationship management. Every feature built needs a corresponding Thumper-accessible explanation.
...
## Bomb Party Jewelry Database — Decided (Sessions #3, #16)
...
### Three-Table Data Model — Decided (Session #16)

**Table 1 — `collections`:** Its own table, NOT a fixed enum. Grows forever as the platform discovers new collections through rep submissions. BP releases many collections constantly — monthly, holiday, specialty, limited drops. Some return annually, some sell out and are gone forever. Each collection is uniquely identified by name + year (e.g., "March 2026" ≠ "March 2025"). Tracks collection type (recurring, seasonal, specialty) and status (active, retired/sold out). Retired collections stay in the database permanently — historical data for rarity tracking.

**Table 2 — `jewelry_designs`:** One row per unique BP jewelry design. This IS the jewelry database / library. Fields: design_id (UUID PK), collection_id (FK to collections), jewelry_type (enum: ring/necklace/earring/bracelet), item_number (nullable text — from BP box/tag if it exists, or NR-assigned tracking number if not), name (nullable text — from BP if it exists), material (enum: brass/sterling/vermeil), bp_msrp (decimal — always on the reveal box, BP's stated value), description (text — standardized from OCR), canonical_photo (url — best available photo), attributes (jsonb — size, stone type, style descriptors), is_unicorn (boolean), is_diamond (boolean), limited_quantity (nullable int — from packaging if stated), times_listed (int counter), created_by_rep_id (FK — first contributor), created_at (timestamp).

**Table 3 — `trade_listings`:** One row per individual listing on a rep's trade board. Many listings → one design. Fields: listing_id (UUID PK), rep_id (FK), design_id (FK to jewelry_designs), listing_photo (url — rep's specific photo), status (enum: available/traded/sold), price_asked (decimal nullable), trade_preferences (text), reservation (jsonb), listed_at (timestamp), resolved_at (timestamp).

**MSRP — single field, BP's number only (Session #16).** No separate "market value" or "real value" field. Using BP's stated MSRP avoids friction with Bomb Party. One field: bp_msrp.

**Rarity — tags only, no scoring at launch (Session #16).** Unicorn and diamond boolean tags available from day one (BP explicitly marks these during reveals). Limited production runs captured if printed on packaging. Broader rarity scoring (frequency analysis, market demand patterns) is a post-launch feature that emerges from accumulated trade data.

### Deduplication Strategy — Decided (Session #16)

**Matching flow at submission time (Thumper conversation):**
1. Rep submits photo(s) + description to Thumper
2. Thumper runs pre-flight check (photo quality)
3. Thumper extracts structured attributes from reveal box photo (vision/OCR): collection, jewelry type, material, item number (if visible), name (if visible), BP MSRP
4. System searches `jewelry_designs` — exact match on item_number first (golden key). If no item number, fall back to collection + type + material + MSRP. If still no exact match, fuzzy search on description.
5. If candidate found → Thumper shows rep the existing design: "This looks like [name + photo]. Same piece?" Rep confirms or says no. Multiple candidates = show top 3.
6. Match confirmed → new listing links to existing design. Counter increments. Better photo offered as upgrade.
7. No match → new design row created. New listing links to it. Rep is first contributor.

**Key principles:** Rep confirms the match (not fully automated). Text-first matching at launch; visual similarity matching is parking lot (Phase 2). Collection + type + material together define the context — same ring from different collections = different designs. If BP doesn't provide an item number consistently, NR assigns its own tracking number.

### Filtering Requirements — Hard Requirement (Session #16)

Robust filtering on both the jewelry library (platform-wide catalog) and individual rep trade boards. This is a hard requirement, not a nice-to-have.

**Minimum filter set at launch:** Collection (scrollable/searchable — list grows forever), jewelry type (ring/necklace/earring/bracelet), material (brass/sterling/vermeil), BP MSRP range (slider or bracket), rarity tags (unicorn, diamond), size (especially rings — Kids/5/6/7/8/9/10/11), availability status (trade board only: available vs. gone).

**Extensible by design** — filter system must support adding new filter categories without code changes. Future filters may include: stone type, style descriptors, collection year, retirement status, listing count.

**Mobile-first filtering:** Horizontal scrollable pill bars for primary filters, expandable secondary filter panel for deeper drilldown. Same pattern on both library and trade board — consistent UX.

---

## Trade Board — Decided (Sessions #3, #11, #16)
...
**Trades only at launch (Session #16).** No buying off the trade board. No "pay the difference" trade-ups. No cash transactions through the board. Equal or lesser MSRP swaps only. Buying and trade-ups are future problems.
...
1. Customer gets a reveal they don't want during a live show
2. Rep tells customer: "Go to my trade board, find something equal or lesser MSRP to what you got"
3. Customer goes to the rep's site, browses trade board, clicks "I want this" on a piece
4. Reservation form captures what they're offering — at minimum the MSRP from their reveal box, description or photo
5. The moment they submit, Thumper pings the rep in real time: notification with the trade details (customer name, their piece MSRP, requested piece and its MSRP)
6. Rep approves or rejects with a quick tap — show keeps rolling
7. If approved: the traded piece DISAPPEARS from the trade board immediately. No "reserved" status for trades. Either available or gone.
8. After the show: rep does full batch processing. Catalogs new pieces properly (photos, descriptions, design matching). Thumper walks them through pending items.

**Value matching:** Equal or lesser MSRP. Rep configures trade tolerance once ("equal value only" or "within $X"). Customer self-reports their MSRP (printed on the box). System checks against listed piece's MSRP. If outside tolerance, customer sees a message. Rep can always override manually. Rep verifies everything during post-show batch processing. Customers train themselves over time.

**Thumper live show presence (Gap 23 — open):** During live shows, reps already have the BP dashboard taking up their screen. All the rep needs during a show is Thumper — a lightweight, always-on presence that pops notifications and lets them approve/reject with a tap. Not the full dashboard. Delivery mechanism (floating widget, PWA pop-out, browser extension panel, mobile second screen) needs research and design decision.

---

## SEO/GEO Strategy — Research Complete (Session #16)

Gap 15 / OQ-28 research sprint completed. Gemini deep research received and analyzed. No architecture-breaking findings. Key outcomes:

**Duplicate content (Pages 3 & 4):** Not a penalty risk — "cluster filtering" where Google picks one version. Mitigation: localized content wrappers per rep ("Welcome to Jane's Denver FAQ"), independent canonical tags per custom domain, dynamic LocalBusiness schema per rep. Pages 1 & 2 being highly unique provides sufficient domain uniqueness.

**Schema.org upgrades for build spec:** Add LocalBusiness (areaServed, geo), VirtualLocation (for streaming events), Event (eventAttendanceMode: OnlineEventAttendanceMode), ProfessionalService (brand). These drive "rep near me" and event search visibility.

**Rendering strategy confirmed:** ISR for dynamic pages (homepage, About — trade board/calendar freshness). SSG for locked pages (U&D/FAQ, Join Team — build-time cache). SSR only for logged-in rep dashboard (not indexed).

**Crawl budget:** Google allocates per domain, not per deployment IP. 100 custom domains = 100 separate crawl budget buckets. Dynamic sitemap.xml and robots.txt per hostname required (Next.js host header detection).

**AI discoverability:** AI crawlers treat custom domains on shared deployment as independent sites. llms.txt per custom domain with rep-specific content. Dynamic trade board data surfaced via CollectionPage + Product schema.

**Markdown for Agents — native implementation confirmed:** WASM engine (@kreuzberg/html-to-markdown-wasm) for Edge Runtime, ~3x faster than JS libraries. On-the-fly conversion for dynamic pages. Content-Signal header: ai-train=no, search=yes, ai-input=yes. Protects rep content while enabling AI search.

**Google Business Profile:** Reps should be encouraged to create GBP linking to their custom domain. NAP consistency is the driver. Rep education item, not a build item.

**B2B content hub for yoursparklesuite.com:** Needed for ranking beyond brand terms. PARKING LOT — not a launch requirement.

**Jewelry database as grounding data:** Product + CollectionPage schema on catalog. Network effect compounds search visibility at scale. P3 priority — future customer-facing phase.

**Priority matrix:** P0 = dynamic sitemaps/robots.txt per domain + host-based routing. P1 = localized schema generation + llms.txt per domain. P2 = native markdown serving + B2B content hub. P3 = central catalog indexing.

---

## Lindsey Prototype — Decided (Session #16)

Trade board prototype for Lindsey MUST include Thumper. The chatbot-first experience is the entire value proposition. A trade board without Thumper is just another static webpage with a form — testing without it would be testing the wrong thing.

**Prototype scope:**
- Trade board page on her site with full filtering (collection, type, material, MSRP, size)
- Thumper as rep-facing interface: add pieces, receive trade notifications, approve/reject trades
- Customer-facing "I want this" flow with MSRP entry
- Real-time trade request notifications through Thumper during live shows
- Approved pieces disappear from board immediately
- Post-show batch processing via Thumper walkthrough

**Included from Thumper architecture:** Claude API (Haiku 4.5), Vercel AI SDK streamText with tool-calling, subset of trade board tools, concise/warm personality, Supabase RLS (build it right from day one even for one rep).

**NOT in prototype:** Full dashboard, AI photo enhancement, deduplication logic (database starts empty), multi-rep features, full library view, voice interface, SMS/email automation.

**Validation goal:** If Lindsey can run a live show and manage trades through Thumper without needing Louis, the concept is validated.

**Pre-build blockers (Gaps 20–23):** Four items must be resolved before Claude Code touches the prototype. See SS_KB_OpenItems_v1.6.
...
**All legal items = research sprints.** No legal language written without a Gemini research sprint first. See SS_KB_Legal module and SS_KB_OpenItems_v1.6 legal sprint queue.
...
| April 9, 2026 | Gap Analysis #16 | Gap 15 research complete (SEO/GEO — no architecture-breaking findings). Gap 16 design complete (three-table data model, dedup strategy, live show trade flow, filtering requirements). MSRP = single field (BP's number). Rarity = unicorn/diamond tags only at launch. Trades only at launch (no buying). Lindsey prototype scope decided (must include Thumper). Gaps 20–23 added as pre-build blockers. Standing Rule 15 added. |
