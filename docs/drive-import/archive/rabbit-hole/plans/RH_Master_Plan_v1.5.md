...
**Version:** 1.5 | **Created:** April 5, 2026 | **Last Updated:** April 6, 2026 | **Status:** ACTIVE
...
6. **Agent-ready architecture** — AI agents can discover, purchase, and configure the app on behalf of users. MCP (Model Context Protocol) confirmed as the standard agent interface protocol (Research #7).
...
- **Machine-readable discovery:** Publish `llms.txt` (Markdown at domain root) and `/.well-known/ai` (structured JSON listing all tools, auth requirements, rate limits). Low effort. See OD-29 for whether to publish at Gate 1 launch or Gate 2.

**Agentic commerce integration (Research #7):**

Multiple commerce protocols are active in 2026: ACP (Stripe/OpenAI), MPP (Stripe/Tempo), UCP (Google/Shopify), x402 (independent). The most relevant for Rabbit Hole is MPP — uses HTTP 402 "Payment Required" responses that agents fulfill programmatically. The MCP server should expose a `get_pricing` tool returning structured gate/price data so agents can compare the OTP model against subscription competitors in milliseconds. Backend should support structured payment challenges (HTTP 402 pattern) even if initial checkout is human-facing Stripe. Connects to Research #8 for crypto/stablecoin settlement.
...
**Payment strategy (Research #3, #6, and #7 findings):** Three competing/complementary recommendations that need resolution at decision round:
- Research #3 recommends native in-app purchases (Apple/Google IAP) for all purchases under $20 — the 15-30% conversion drop from redirecting to external payment is more damaging than the 15% commission.
- Research #6 identifies a temporary 0% commission window via US anti-steering ruling (Epic v. Apple) for Stripe web checkout — this could make external payment the better option for US users while the window is open.
- Research #7 introduces agentic commerce protocols (MPP, x402) where agents pay programmatically via HTTP 402 challenges — often using stablecoins. Backend should support structured payment challenges regardless of primary checkout flow. See OD-26.
- Crypto/stablecoin payments under research (Research #8).
...
Free tier limits enforced. One-time purchase unlocks full capacity. Payment method decision (native IAP vs. Stripe web checkout vs. both) depends on OD-26 resolution. Tier flag on rh_users.
...
An 8-part deep research sprint is underway before any Rabbit Hole code gets written. Decision round happens after all 8 are analyzed — organized by gate (Gate 1 first, then Gate 2, then Gate 3). After the decision round, this master plan will be split into a focused Gate 1 spec (Layer 1 candidate) and a full master plan (Layer 2 reference). Key findings and banked decisions are captured in Open Brain under "RESEARCH FINDINGS SUMMARY."
...
| 7 | Agent-Friendly Architecture | ✅ Complete — analyzed | Open Brain + RH_Research_07 (generate for Drive) |
| 8 | Alternative Distribution + Crypto Payments | Gemini results in hand — analysis next session | — |
...
| OD-14 | Crypto/stablecoin payment integration — scope and timing | Research #8 (Alternative Distribution + Crypto Payments); Research #7 identifies MPP and x402 as implementation paths | After Research #8 |
...
| OD-26 | US anti-steering payment strategy — Stripe web checkout (0% commission, temporary) vs. native IAP (15%, permanent) vs. both | Legal timeline uncertainty + Research #3 tension; Research #7 adds HTTP 402 pattern for agentic commerce | Decision round |
...
| OD-28 | `intent_metadata` JSONB column on `rh_subjects` — add in Gate 1 (nullable, zero-cost future-proofing for agent instructions per Rabbit Hole) or defer to Gate 2? | Schema decision | Decision round — NEW from Research #7 |
| OD-29 | Machine-readable discovery files (`llms.txt` and `/.well-known/ai`) — publish at Gate 1 launch or defer to Gate 2? Research #7 recommends launch-day for discoverability. | Timing decision | Decision round — NEW from Research #7 |
| OD-30 | Agent state tables (`agent_sessions`, `tool_invocations`) — include in Gate 2 schema or defer to later? | Gate 2 scope decision | Gate 2 planning — NEW from Research #7 |
...
| RG-13 | What are viable alternatives to app store distribution and traditional payment processing? | Research #8 — Alternative Distribution + Crypto Payments | Gemini results in hand — analysis next session |
...
12. **Compliance-first:** Age verification, API target levels, AI labeling, App Store 4.2.2 mitigations, privacy nutrition labels — built in from day one, not bolted on later.
...
14. **Agent-ready:** Build for humans today and AI agents tomorrow. The app must be discoverable, purchasable, and configurable by machines, not just humans. MCP is the standard protocol. Schema decisions in Gate 1 determine Gate 2 agent feasibility. (Updated from Research #7)
