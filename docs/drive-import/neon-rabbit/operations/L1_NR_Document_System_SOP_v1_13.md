...
**Version:** 1.13 | **Created:** April 5, 2026 | **Last Updated:** April 22, 2026 | **Status:** LOCKED
...
## v1.13 Changes (April 22, 2026 — late session diagnostic)

Triggered by April 22, 2026 evening session quality failures (three Claude misses + premature session-close attempt). Late-session diagnostic identified two structural problems and one cultural shift. All captured as RULE REVISION entries per Rule 30 discipline before file generation.

- **NEW:** **Session Triage Step** added to Session Open Protocol. After the 6 pulls complete, Claude runs a 4-line snapshot triage of agenda items, heavy artifacts, load-bearing decisions, and a recommendation. **Default posture is default-to-propose-split** when triage detects any heaviness. Louis can override with "run as-is." Reasoning: quality over speed, no deadlines except VAC easy stuff. Better to take longer with cleaner sessions than push through and lose accuracy. Applied retroactively to April 22 evening would have flagged that session as three sessions, not one.
- **UPDATED:** **Restart Prompt format — no reference file uploads at session start.** Two file categories now: (1) auto-loaded L1 files (SOP, Standing Rules, Memory System Map — come with the project, no upload), (2) on-demand uploads requested by Claude during the session as agenda items actually surface. Exception: when the session has ONE artifact that IS the work, upload at start. Rationale: pre-uploading 4-6 reference files burns token budget and creates description-collapses-to-source collisions when most files never get read.
- **UPDATED:** Session Close Protocol section — adds explicit reminder that **session close requires Louis authorization between each part**. Captures → file generation → restart prompt is not an automatic chain. Each part requires a Louis-side go-ahead. This codifies what was already implicit in Rule 9 + Rule 18 + the April 21 session-close-discipline capture.
...
| File | Status |
|---|---|
| `L1_NR_Document_System_SOP_v1.13.md` | ✅ Live (this file) |
| `L1_NR_Plugins_Skills_Standing_Rules_v3.17.md` | ✅ Live |
...
## Session Open Protocol (UPDATED — v1.13: 6 pulls + triage step)
...
**5. (Rule 29 — v3.17 amendment) Scoped `CLAUDE —` engineering pull** — based on expected session domain(s), threshold 0.4, limit 5, maximum 2 domains per session. **"session management" domain MUST be included as one of the (max 2) domains for the FIRST output of any new session, regardless of stated session topic.** This addresses the session-open paradox where session-open-drift lessons are systematically not retrieved when most needed.

Examples (with session management always present for first output):
- Architecture decision session → pull `session management` + `architecture`
- Design mapping session → pull `session management` + `design sessions`
- Code prompt writing session → pull `session management` + `prompt writing`
- Workflow/efficiency tune-up session → pull `session management` + `communication`

Targeted engineering-lesson retrieval only.

**6. (Rule 31) `CLAUDE ABOUT LOUIS —` partnership pull** — always pulled, NOT domain-scoped, threshold 0.4, limit 3.
...
**7. (NEW — v1.13) SESSION TRIAGE** — After pulls complete, BEFORE touching any agenda item, Claude runs a 4-line snapshot triage:

```
SESSION TRIAGE:
- Agenda items detected: [list from restart prompt]
- Heavy artifacts in scope: [files uploaded, large reference docs, schemas, multi-decision specs]
- Load-bearing decisions in scope: [decisions with downstream consequences for other work]
- Recommendation: [proceed as-is / propose split into N sessions]
```

**Default posture: default-to-propose-split when triage detects ANY heaviness.** Better to take longer with cleaner sessions than push through and lose accuracy. Louis can override with "no, run as-is" — but the default surfaces the option BEFORE the heavy lift starts, when budget remains for redesign.

The triage is snapshot form (Rule 14), not essay. If the answer is "single-shot OK," proceed in seconds. If the answer is split-recommended, propose the split BEFORE starting work.
...
Pull #4 gives Claude live build reality from Supabase in one call. Open Brain pulls (1-3 + 5 + 6) carry decisions, context, engineering lessons, partnership context, and concerns that structured data cannot capture. Step 7 governs how those inputs translate into session shape.

**Reference file uploads at session start (UPDATED v1.13):**

L1 files are pre-loaded automatically (SOP, Standing Rules, Memory System Map — come with the project, no upload needed).

**No additional reference file uploads at session start by default.** When the agenda hits an item that genuinely needs a reference doc (master plans, architecture specs, schemas, restructure plans), Claude asks: *"Pulling Item N — need [filename] uploaded for this."* Louis uploads then, fresh, against a clean head.

Exception: If the session has ONE artifact that IS the work (e.g., a schema review session where the schema doc IS the subject), upload at start. But this is a specific signal, not the default.

Rationale: Pre-uploading 4-6 reference files when the session typically only reaches the first 1-2 items is pure context cost with zero benefit — and creates description-collapses-to-source collisions when pulled-but-unread context fuses with whatever does get read.

After completing all 7 steps (pulls + triage), confirm status back to Louis before touching any task:

> "✅ Session open complete. Here's where we stand: [summary]. Build status: [from MCP]. Active tasks: [list]. Relevant CLAUDE — engineering learnings: [short summary if any]. Relevant CLAUDE ABOUT LOUIS — partnership context: [brief summary if any]. Triage: [proceed-as-is / split-proposed]. Next action: [specific first step]. Anything missing or wrong?"
...
**(v1.13 reinforcement) Louis authorization required between each part.** Session close is not an automatic chain. Captures → file generation → restart prompt requires a Louis-side go-ahead between each step. Claude does not chain from "captures landed" to "generating files" without Louis explicitly authorizing the next part. This codifies what was already implicit in Rule 9 (session close file generation) + Rule 18 (nothing COMPLETE without Louis) + the April 21 session-close-discipline capture. Triggered by April 22 late session where Claude chained capture → file generation without authorization.
...
Examples:
L1_NR_Document_System_SOP_v1.13.md
L1_NR_Plugins_Skills_Standing_Rules_v3.17.md
