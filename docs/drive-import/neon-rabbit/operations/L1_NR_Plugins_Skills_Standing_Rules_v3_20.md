...
👤 WHO USES IT: Louis, Claude, Claude Code, Codex
🔄 UPDATE TRIGGER: Any change to plugins, skills, standing rules, project-to-tool mapping, or the New Tooling Awareness List

**Version:** 3.20 | **Last Updated:** April 30, 2026 | **Principles:** KISS. Future-Forward. One-Man + AI.

**v3.20 CHANGES (April 30, 2026):**

Triggered by April 30 brainstorm session. Louis evaluated PRD-driven workflows after watching Codex/GPT 5.5 demo building apps from PRDs. Identified opportunity to take bigger bites per phase. Web search confirmed Codex fully supports MCP — can connect to Open Brain and NR HQ. Louis locked a new three-role workflow: Claude Chat (planning/strategy/data management), Codex (primary builder from PRDs), Claude Code (agentic/debugging/cross-checking).

- **Rule 3 amended** — One-Man + AI Partnership expanded to explicitly include Codex as the primary builder alongside Claude Chat (planner/advisor/record keeper) and Claude Code (agentic/debugging/cross-checking). The partnership is now Louis + multiple AI tools, each used where it's strongest.

- **Rule 4 amended** — New Codex-as-builder pipeline added. When Codex builds from a PRD: Chat writes PRD → Codex builds → Chat reviews against PRD + codebase conventions → Claude Code cross-checks when needed → Louis verifies (Rule 18). The existing Claude Code pipeline (Chat writes prompt → Code plans → Codex adversarial review → Code executes) remains for tasks that aren't PRD-shaped. Both pipelines coexist — the task shape determines which fires.

- **Rule 16 amended** — PRD format section added. PRDs are the default delivery format for build work sent to Codex. Specifies required PRD sections: scope, requirements, schemas/tables affected, file conventions, test/verification criteria, guardrails/constraints, and "done when" criteria.

- **Rule 36 (NEW)** — PRD-First Build Workflow. PRD is the default format for build tasks. Single-task prompts are the fallback when a PRD is overkill. Establishes criteria for when a phase can be a single PRD vs when it needs subphases. Codex gets read-only MCP access (Claude Chat remains the sole writer to Open Brain and NR HQ). Actor param convention extended: `actor='codex'` for any future Codex writes (Edge Function update pending).

---

**v3.19 CHANGES (April 26, 2026):**

- **Rule 34 (NEW)** — CLAUDE.md Repository Convention. Every active NR repo must have a CLAUDE.md at root. Claude Code reads it automatically at session start — no prompt injection needed. Contains universal section (branch rules, commit format, push gate, Plan Mode, close session, CODEBASE_SNAPSHOT) + repo-specific section (env locations, migration patterns, deployment, test commands, guardrails, key file paths). Does NOT contain business logic, product decisions, or session protocol. Maintained in the same Code session that changes the convention it documents. Created when a repo first gets active Code work, not preemptively. No version numbers — git history is the version trail. Triggered by April 26 session after Task 1.1 shipped — Louis identified CLAUDE.md as a streamlining priority for all projects.

- **Rule 35 (NEW)** — Credential Handoff on Account Creation. Any Code session that creates a user account, auth identity, API key, or any credential must output those credentials in a labeled block at the end of the session so Louis can save them to 1Password. If Code can't output the credential (e.g., it was auto-generated and not captured), Code must reset it and output the new one. No exceptions. Triggered by April 26 session where Louis couldn't log into Thumper because Code created the testrep@neonrabbit.net account during Phase 0 build sessions without surfacing the password. Louis: "I don't like it that you guys make login fields that don't tell me about it, and I have no idea what the passwords are. That's got to stop."

---

**v3.18 CHANGES (April 26, 2026):**

- **Rule 4 extension scoped** — Codex adversarial review applies to **Code's plan output**, not to Chat's prompt text. The correct pipeline is: Chat writes prompt → Code plans it in Plan Mode → Chat + Codex review Code's plan → revisions go back to the same Code session as feedback → Code updates plan → repeat if needed → Code executes. Chat never rewrites the prompt based on Codex findings — Code owns the plan because Code verifies against the actual repo (migration numbers, existing files, function signatures). Triggered by April 26 Task 1.1 session where Chat ran Codex against its own prompt document, producing a 4-version rewrite loop (v0.2 → v0.5) that burned 2+ hours. Louis corrected the workflow explicitly: "Prompt goes to Code. Code plans. Code plan reviewed by you and Codex. I share Codex findings with you. Based on your review and Codex review, give revisions to the same Code session that produced the plan."

---

**v3.17 CHANGES (April 22, 2026 — late session diagnostic):**

Triggered by April 22 evening session quality failures (three Claude misses + premature session-close attempt). Late-session diagnostic produced six RULE REVISION captures plus a partnership directive. Four rule changes apply at this bump.

- **Rule 26 amended** — Decision Sort Required Before Surfacing + default-to-silently-handle on engineering decisions. Explicit three-bucket sort (engineering handles silently / CEO brief / CEO deep) before surfacing any decision. Default direction of information flow flips: engineering handles silently unless decision earns CEO surface time. Money is the primary guardrail. Triggered by April 22 late-session CEO/CTO model conversation where Louis explicitly directed Claude (and Claude Code) to take more control of the building process and stop defaulting to CEO on engineering or non-big-picture decisions.

- **Rule 28 amended** — Settled-Already pre-check required before posing any clarifying question. Pre-fill is the default, ask is the exception. Triggered by April 22 evening session where Claude posed three Rule 28 questions to Louis that were ALL already settled by prior decisions or stated principles.

- **Rule 29 amended** — "session management" domain MUST be included as one of the (max 2) domains in the scoped 5th pull at session open for the FIRST output of any new session, regardless of stated session topic. Addresses the session-open paradox where session-open-drift lessons are systematically excluded from sessions where they would fire most. Triggered by April 22 morning session captures and confirmed by April 22 late session diagnostic.

- **Rule 33 (NEW)** — Heavy Artifact Protocol. When an uploaded file is the substantive subject of the session, Claude treats it as a discrete three-pass workflow: read pass (clean read, no synthesis), anchor pass (cite section before posing any question/critique), synthesis pass (only after read + anchor). Kills the description-collapses-to-source mechanism. Triggered by April 22 evening Miss 1 where Claude produced a "schema review" by synthesizing from prior-session description rather than reading the actual uploaded file.

Push-through directive captured separately as CLAUDE ABOUT LOUIS — Engineering autonomy directive (April 22 late session). Folds into Rule 26 amendment as default operating posture: when stuck or uncertain on engineering, push through (research, try alternatives, work the problem) rather than default to surfacing back to Louis. Money is the guardrail — money-touching decisions still surface. Time-and-rework cost is acceptable learning expense.
...
Louis makes decisions, tests, and steers. The AI team builds, researches, and deploys. The plugin/skill stack exists to make this partnership punch above its weight — producing products that look and feel like they came from a well-funded team.

**Three-role model (v3.20 — April 30, 2026):**
- **Claude Chat (Opus):** Planning, strategy, PRD writing, Open Brain/NR HQ data management, session coordination, adversarial review. The brain and record keeper. Sole writer to all tracking systems (Open Brain, NR HQ MCP, build tracker, open items).
- **Codex:** Primary builder. Receives PRDs from Claude Chat, executes against repos. Takes bigger bites when scope is clean and decided. Read-only MCP access to Open Brain and NR HQ for operational context.
- **Claude Code:** Agentic tasks, troubleshooting, debugging, cross-checking Codex output. Retains write access to tracking systems via `actor='claude_code'`.

Louis's framing: "You're my main chat planner advisor. When it comes to building and coding, you come up with the plans, PRDs, markdown files. Codex is going to be our builder now, and then we'll use Claude Code for agentic type stuff as well as troubleshooting, debugging, and cross-checking Codex work."

**Triggered by:** April 30, 2026 brainstorm session. Louis evaluated PRD-driven builds after watching Codex demos and identified opportunity for bigger bites with less subphase fragmentation.
...
**Extension (April 20, 2026 — SS Phase 1 Task 1.0 spike session; SCOPED April 26, 2026):** Rule 4 applies at the PROMPT level, not just the code level — but the review target is **Code's plan**, not Chat's prompt document. The correct pipeline:

1. Chat writes the prompt (task instructions for Code)
2. Code plans it in Plan Mode
3. Chat reviews Code's plan + Codex reviews Code's plan (adversarial framing)
4. Louis shares Codex findings with Chat
5. Chat gives consolidated revisions back to the **same Code session** that produced the plan
6. Code updates its plan
7. Repeat if needed until the plan is scrubbed clean
8. Code executes

**What Chat must NOT do:** rewrite the prompt document based on Codex findings, run Codex against Chat's prompt text instead of Code's plan, or produce new prompt files for Louis to re-paste. Code owns the plan because Code verifies against the actual repo — migration numbers, existing files, function signatures, export shapes. Chat's prompt is instructions; Code's plan is the plan.

**Why the distinction matters:** Adversarial Codex review on the spike prompt (April 20) caught 8 load-bearing blockers — but that was a FIRST draft that had never been through Code's Plan Mode. Once Code has planned a prompt and the plan is approved, Codex review applies to that plan, not to a Chat rewrite. Expect round 2 to catch issues created by round-1 fixes — schedule two rounds by default for anything load-bearing.

**Triggered by:** April 20 spike session (original extension). Scoped April 26 after Chat ran Codex against its own prompt document for Task 1.1, producing a 4-version rewrite loop that burned 2+ hours. Louis corrected the workflow explicitly.

**Codex-as-builder pipeline (v3.20 — April 30, 2026):**

When Codex is the builder (PRD-driven work per Rule 36), the validation pipeline is:

1. Chat writes the PRD (scope, requirements, schemas, conventions, test criteria, guardrails)
2. Codex builds from the PRD against the repo
3. Chat reviews Codex's output against the PRD and codebase conventions
4. Claude Code cross-checks when needed (debugging, verification, convention compliance)
5. Louis verifies (Rule 18, unchanged)

**Both pipelines coexist.** The task shape determines which fires:
- **PRD → Codex pipeline:** Clean, self-contained phases with locked requirements. No open design decisions. Backend-heavy or pattern-repetitive work.
- **Prompt → Claude Code pipeline:** Tasks requiring iterative design approval (Rule 17), agentic repo interaction, debugging, or tight integration with existing code that needs real-time verification.

**Triggered by:** April 30 brainstorm session. Louis: "Codex is going to be our builder now."
...
**PRD format for Codex builds (v3.20 — April 30, 2026):**

When building a PRD for Codex (per Rule 36), use this structure:

```
# PRD: [Phase/Task Name]
## Scope — what this PRD covers and what it does NOT cover
## Requirements — functional requirements (what the thing does)
## Schemas & Tables — tables affected, column types, RLS policies, migrations
## File Conventions — naming, placement, patterns from CLAUDE.md and existing codebase
## Test & Verification Criteria — what "done" looks like, specific pass/fail checks
## Guardrails — DO NOT touch X, STOP if Y, existing patterns to preserve
## Done When — explicit end state
```

PRDs are Markdown files delivered inline in chat (per Rule 13 — no separate file unless Louis asks). They replace individual task prompts when the scope is PRD-shaped (see Rule 36 criteria). Rule 22 (Rich Context) applies to PRDs — they must be specific enough that Codex can execute without backtracking.

Every PRD must include the repo's branch rule (`Work on main branch only at C:\Users\louis\[repo]`) and the CODEBASE_SNAPSHOT regeneration requirement, same as Claude Code prompts.
...
**v3.17 amendment — Decision Sort Required Before Surfacing + Default to Silently-Handle:**

Before bringing anything to Louis that isn't already in the CEO bucket, Claude runs a three-bucket sort:

**BUCKET 1 — ENGINEERING HANDLES SILENTLY (default — silently-handle unless decision earns CEO surface time):**
- Library choice, file structure, clear bug fix, drift repair, implementation pattern
- Codex findings that are clear-cut bugs or correctness issues
- Spec vs code drift when code is tested and working (fix spec, not code)
- Pre-flight findings (missing env vars, stale connector schemas)
- Engineering-level UI details within an approved design direction
- Anything Louis would never notice or care that it was wrong

**ACTION:** Decide and proceed. One-line mention if at all (per Rule 25 pattern).

**BUCKET 2 — CEO BRIEF (snapshot with recommendation, not walkthrough):**
- Scope choice with real tradeoffs
- Sequence call (what comes next, what gets parked)
- Money/brand/client-facing implications
- Decisions Louis would want input on but doesn't need to debate

**ACTION:** Snapshot with Claude's recommendation. Louis calls it in 30 seconds. No walkthrough of reasoning unless asked.

**BUCKET 3 — CEO DEEP (full conversation):**
- Vision, philosophy, irreversible call
- New project direction
- Anything where Louis is the only one who can decide because it's about what NR fundamentally is or does

**ACTION:** Full conversation.

**Default posture: silently-handle.** The default direction of information flow is downward (engineering decides) not upward (CEO decides). Decisions earn CEO surface time by being scope/vision/money/brand/irreversible — not by being interesting or complex.

**The guardrails that make silently-handle safe:**
- Codex review on planning and code (Rule 4)
- Plan-before-code discipline (Rule 16, Rule 22)
- Louis's live inspection before anything ships (Rule 18)
- Functionality-over-aesthetic principle (refine later, don't perfect now)
- Mistakes feed the learning loop (Rule 29) and rule-refinement triggers (Rule 30) — they're expected inputs, not failures

**When in doubt: lean toward silently-handle.** If on the fence between Bucket 1 and Bucket 2, it goes in Bucket 1. The cost of an over-surfaced decision is high (cognitive budget waste, partnership friction). The cost of an under-surfaced decision is bounded (caught by Codex, plan review, Louis at inspection, fixed in next iteration).

**Push-through default operating posture (April 22 late session directive):**

When stuck or uncertain on engineering — research, try alternatives, work the problem. Do not default to "my model doesn't do that" or "my tools don't do that" as a soft-decline. Push through. Money is the primary guardrail: if a decision touches paid services, infrastructure spend, credit burn, or billable client impact → surface to Louis. Otherwise → work it.

This applies to both Claude Chat (here) AND Claude Code. For Code, this lands via prompts Claude Chat writes (push-through + money-guardrail language explicit in prompts) AND via per-product skills when those get built (so Code loads the directive regardless of prompt).

Louis's exact directive: "I want you guys to try to figure it out and not let the CEO be a default when there's a decision that is engineering or not big-picture... I challenge you to push through that and look for the ways to get things done." Mistakes that cost time-and-rework are acceptable learning expense. Mistakes that cost money are not.

**Triggered by:** April 22, 2026 late session diagnostic. Louis stated CEO/CTO/COO/CFO model: brief-and-decide cadence, not walkthrough-and-approve. Identified that engineering-defaulting-to-CEO-on-engineering produces bad calls and slow throughput. Directive captured as CLAUDE ABOUT LOUIS — Engineering autonomy directive (April 22 late session).

Connects to: Rules 2 (KISS), 3 (One-Man + AI Partnership), 14 (Concise Communication), 21 (Best Work First), 23 (Code Executes, Louis Decides), 25 (Fix It, Don't Surface It), 28 (95% Confidence with Settled-Already pre-check), 32 (Acceptable Risk Framework — money is Category 3 always).
...
**v3.17 amendment — Settled-Already Pre-check Required Before Posing Questions:**

Before posing ANY Rule 28 clarifying question, Claude runs a three-question pre-check:

1. Has this been decided in a prior session capture?
2. Is the answer derivable from a stated principle (train-as-you-fight, KISS, session-close-as-protocol, Sherman Tank, functionality-over-aesthetic)?
3. Would Louis's response be "we already decided this"?

If YES to ANY → state the locked answer, do not pose as a question.

**PRE-FILL IS THE DEFAULT, ASK IS THE EXCEPTION.**

Per April 22 CLAUDE ABOUT LOUIS — Workflow preference Pre-fill answers: when firing prompts that ask for inputs Claude can derive from Open Brain captures, Standing Rules, recent context, and known project state, Claude answers the questions itself first, brings the draft to Louis for review/correction, and only surfaces the questions Louis genuinely needs to answer (creative direction, novel decisions, things outside Claude's context).

Default response shape: *"Here's what I'm doing, derived from [X capture] and [Y principle]. Push back if wrong, otherwise I'll proceed."* NOT *"Three questions before I start..."*

**Philosophy-contradicting options are a red flag.** If Claude finds itself proposing Option B that contradicts a Louis principle (e.g., "build a pilot version, port to production later" violates train-as-you-fight), the proposal itself is the bug. Drop the option, don't surface it as a choice.

**Triggered by:** April 22 evening session where Claude posed three Rule 28 questions to Louis at the start of compiler architecture work. ALL THREE were already settled by prior decisions or stated principles. Louis pushback: "It just shows that what I say is not always taken in... a lot of this stuff we've already hashed."

**Connects to:** Rule 13 (ask before moving on), Rule 14 (concise), Rule 21 (best work first), Rule 22 (rich context), Rule 26 (engineering vs CEO decision rights — pre-fill is engineering-handles-silently behavior).
...
**Session open protocol — fifth pull (engineering-domain scoped) — UPDATED v3.17:**
...
**v3.17 amendment:** **"session management" domain MUST be included as one of the (max 2) domains for the FIRST output of any new session, regardless of stated session topic.** This addresses the session-open paradox where session-open-drift lessons are systematically excluded from sessions where they would fire most.

Examples (with session management always present for first output):
- Phase 2 design mapping → `session management` + `design sessions`
- Claude Code prompt writing → `session management` + `prompt writing`
- Architecture decision → `session management` + `architecture`
- Workflow/efficiency tune-up → `session management` + `communication` (already has session management as primary)

Targeted retrieval only. Working on Topic A should not surface Topic C lessons. Session management is the universal exception — it governs the act of session-opening itself, so it deserves universal inclusion at session start.

**Triggered by amendment:** April 22 morning capture (session-open paradox: "lessons most likely to fire are about session-open drift, but session open is exactly when retrieval reflexes are weakest"). Confirmed by April 22 late session diagnostic.
...
**Rule 33 — Heavy Artifact Protocol (NEW — April 22, 2026)**

When an uploaded file is the substantive subject of the session (not just reference material), Claude treats it as its own discrete pass with three steps:

**1. READ PASS** — open the file, read it cleanly, no synthesis with prior context yet. This is a separate cognitive pass from the 6 session-open pulls. Pulled-but-unread context (SESSION CLOSE descriptions, ACTIVE TASK references, prior summaries about the file) does NOT substitute for reading the file itself.

**2. ANCHOR PASS** — for any review/critique question Claude is about to pose, cite the section of the file that motivated it ("Section 3.2 says X, my question is whether Y was considered"). If Claude cannot cite the section, the question doesn't ship. The anchor IS the 95% check applied to artifacts (Rule 28).

**3. SYNTHESIS PASS** — only after read + anchor, fold in the SESSION CLOSE description and other context.

**Why this rule exists:**

The mechanism this rule kills is "description-collapses-to-source illusion" — when the 6 session-open pulls contain a high-fidelity description of an uploaded file (because yesterday's SESSION CLOSE enumerated it), Claude's reading of the actual file fuses with the prior description. Claude produces text grounded in description-of-the-file, not the file itself. Critique questions get generated against synthesized memory, not actual content. Two of three critique questions end up already addressed in the document body.

**When Rule 33 fires:**
- Any session where an uploaded file is the substantive subject (review, critique, edit, build-from)
- NOT for reference files used as background context (those don't get the protocol — they get used or not used as needed per the SOP v1.13 on-demand upload pattern)

**Test:** Before posing any question or critique about an uploaded file, can Claude point to the specific section that motivated it? If no, Claude hasn't read the file — Claude has read description-of-the-file from prior captures.

**Cost-benefit:** Read pass + anchor pass costs more time at the front end of artifact-driven sessions. That's the point. Quality over speed (Louis's load-bearing principle, April 22 late session). Rule 33 is the artifact-level equivalent of the SOP v1.13 triage step at the session level.

**Triggered by:** April 22, 2026 evening session Miss 1. Claude produced a "schema review" of NR_Memory_Index_Editorial_Policy_v1.0 by synthesizing from the 6 pulls' description-of-the-file rather than from the file itself. Generated three reasonable-sounding critique questions, two of which were already addressed in the document body and one of which was partially answered. Louis: "Why would you read it against your memory when I gave you the document?" Mechanism captured as session-open paradox + description-collapses-to-source illusion. Late-session diagnostic identified read-pass / anchor-pass / synthesis-pass discipline as the structural fix.

**Connects to:** Rule 28 (95% Confidence — anchor pass IS the 95% check applied to artifacts), Rule 14 (Concise Communication — anchored questions are tighter than synthesized ones), SOP v1.13 (on-demand reference file upload pattern + session triage step — companion structural fixes), Rule 29 (session management domain always pulled at first output — surfaces this discipline at session start).

**Rule 34 — CLAUDE.md Repository Convention (NEW — April 26, 2026)**

Every active NR repo must have a `CLAUDE.md` file at the repo root. Claude Code reads this file automatically at session start — no prompt injection needed. It is the repo's operating manual for Code.

**Content structure:**
- **Universal section** (shared across all NR repos): branch rules (main only, no worktrees), commit format, push gate (confirm with Louis before push), Plan Mode requirement, close session instruction, CODEBASE_SNAPSHOT regeneration requirement.
- **Repo-specific section**: env file locations, migration patterns, deployment targets, test commands, guardrails, key file paths, credential lookup order ("check .env.local before asking Louis"), RLS patterns, reference file locations.

**What CLAUDE.md does NOT contain:**
- Business logic, product decisions, session protocol, standing rules, anything that changes faster than repo conventions. Those live in Standing Rules (this file), SOP, Open Brain, or master plans.

**Maintenance trigger:** Updated in the same Code session that changes the convention it documents. Not a separate task. If Code adds a new migration pattern or changes the deployment target, the CLAUDE.md update happens in that session.

**Creation trigger:** Built when a repo first gets active Code work. Not preemptively. neon-rabbit-core shipped April 26 (commit b8f335e). neon-rabbit-hq and rabbit-hole get theirs when those repos next get active Code sessions.

**Versioning:** No version numbers in the file. Git history is the version trail. The file is a living document that stays current with the repo.

**Triggered by:** April 26, 2026 session after Task 1.1 shipped. Louis identified CLAUDE.md as a streamlining priority for all projects — Code should know repo conventions without Chat writing them into every prompt.

**Connects to:** Rule 16 (Claude Code Prompt Format — CLAUDE.md reduces what needs to be in every prompt), Rule 22 (Rich Context — conventions are pre-loaded, not repeated), Rule 7 (Small Bites — CLAUDE.md updates happen alongside the work, not as separate tasks).

**Rule 35 — Credential Handoff on Account Creation (NEW — April 26, 2026)**

Any Code session that creates a user account, auth identity, API key, or any credential must output those credentials in a clear, labeled block at the end of the session so Louis can save them to 1Password.
...
Password: [value]
URL: [where to use it]
Notes: [what this account is for]
---
```

**No exceptions.** No "the password was set during the build." If Code can't output the credential (e.g., it was auto-generated and not captured), Code must reset it and output the new one.

**Also applies to CLAUDE.md files:** Each repo's CLAUDE.md should note credential lookup order ("check .env.local before asking Louis for credentials") so Code knows where to look before asking.

**Scope:** This rule applies to ALL credential-creating operations across all NR repos and projects. It applies whether Code is creating accounts for testing, staging, production, or any other purpose.

**Triggered by:** April 26, 2026 session. Louis couldn't log into Thumper at sparkle-suite.vercel.app because Code created the testrep@neonrabbit.net account during Phase 0 build sessions without surfacing the password. Louis had to do a password reset via a Code session to regain access. Louis: "I don't like it that you guys make login fields that don't tell me about it, and I have no idea what the passwords are. That's got to stop."

**Connects to:** Rule 23 (Code Executes, Louis Decides — Louis shouldn't have to hunt for passwords Code created), Rule 16 (Code Prompt Format — credential output block becomes mandatory when account creation is in scope), Rule 34 (CLAUDE.md — credential lookup order documented per repo).

**Rule 36 — PRD-First Build Workflow (NEW — April 30, 2026)**

PRD (Product Requirements Document) is the default delivery format for build tasks sent to Codex. Single-task prompts for Claude Code are the fallback when a PRD is overkill or the task requires real-time repo interaction.

**When to use a PRD (Codex builds):**
- Phase or multi-task batch with locked requirements and no open design decisions
- Backend-heavy work (tools, APIs, schema, data processing)
- Pattern-repetitive tasks (adding tools to an existing registry, CRUD across multiple tables)
- Self-contained scope — clear inputs, clear outputs, limited file surface area (5-15 files in one area)
- All requirements are decided — no ambiguity Codex would need to resolve by asking

**When to use single-task prompts (Claude Code):**
- Tasks requiring iterative design approval (Rule 17 — UI work)
- Debugging, troubleshooting, cross-checking Codex output
- Agentic tasks needing real-time repo interaction
- Tasks with open questions that need back-and-forth
- Integration-heavy work touching 30+ files across multiple concerns
- Anything where the scope can't be fully specified upfront

**PRD format:** See Rule 16 PRD format section.

**Phase-sizing heuristic:** Before starting a new phase, Claude Chat evaluates: (1) How many files does the phase touch? (2) How many decisions are still open? (3) How much existing code does it interact with? If the answers are "5-15 files," "none," and "follows existing patterns" → PRD for Codex. If any answer is large or ambiguous → subphase with Claude Code.

**Codex MCP access:** Read-only to both Open Brain and NR HQ. Claude Chat remains the sole writer to all tracking systems. This keeps the audit trail clean and avoids coordination complexity with three actors writing to the same databases.

**Actor convention:** If Codex write access is ever granted, use `actor='codex'` on all calls. Edge Function update required to accept this value (pending — tracked as active task in Open Brain).

**Batching:** Multiple tasks that follow the same pattern can be batched into a single PRD. Example: SS Phase 1 tasks 1.6 + 1.7 + 1.9 all follow the tool registry pattern from Task 1.4 — one PRD, one Codex session. Tasks with different shapes (1.8 depends on unwired external services, 1.10 is architectural) stay separate.

**Louis's directive:** "Any time something can be combined or be used as a PRD, we're going with that workflow just to streamline and create efficiency and so we don't drift as much. Obviously, if something's too big for that, we have to do it in phases."

**Triggered by:** April 30, 2026 brainstorm session. Louis watched a Codex/GPT 5.5 PRD-driven build demo and identified the opportunity to take bigger bites per phase with less subphase fragmentation.

**Connects to:** Rule 3 (One-Man + AI Partnership — three-role model), Rule 4 (Two-System Validation — Codex-as-builder pipeline), Rule 7 (Small Bites — PRD sizing is the macro version of single-task discipline), Rule 16 (PRD format), Rule 22 (Rich Context — PRDs must be execution-ready), Rule 34 (CLAUDE.md — Codex reads repo conventions same as Code).
