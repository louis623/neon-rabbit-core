...
**Version:** 1.8 | **Created:** April 6, 2026 | **Last Updated:** April 14, 2026 | **Status:** ALL SECTIONS COMPLETE — DASHBOARD RESTRUCTURE IN PROGRESS (Tasks 1–6 complete, Tasks 7–8 pending). NR HQ MCP deployed (Session 1 of 3).
**Supersedes:** HQ_Master_Plan_v1.7.md (retired), all prior versions
...
| NR HQ MCP (Claude Chat Read Access) | ✅ Session 1 complete — 5 build tracking tools deployed April 14, 2026. Sessions 2–3 pending. |
...
| **To-Do / Triage Center** | Priority management, what needs to happen next (handled by Build Tracker action cards) |
...
## NR HQ MCP — Claude Chat Read Access (Deployed April 14, 2026)

### Purpose
Gives Claude Chat direct read access to Supabase build tracking data. Claude sees build reality — not just what was captured in Open Brain or uploaded in Markdown files. Open Brain carries memory (decisions, context, lessons). NR HQ MCP carries reality (what's actually built, what status things are in).

### Architecture
- **Separate Edge Function** from Open Brain MCP (Option B) — memory vs reality are separate concerns
- **Greenfield MCP** — reads tables directly, does not mirror the dashboard's data.ts layer
- **Auth:** Service role + MCP_ACCESS_KEY gate (same pattern as Open Brain)
- **Key delivery:** URL query parameter `?key=` (same pattern as Open Brain connector)
- **All tools are read-only** — SELECT queries only, no writes from Claude Chat
- **Supabase client:** Anon key + public SELECT RLS

### Endpoint
`https://bqhzfkgkjyuhlsozpylf.supabase.co/functions/v1/nr-hq-mcp`

Source: `sparkle-suite/supabase/functions/nr-hq-mcp/index.ts` + `deno.json`

Claude.ai connector: Added as "NR HQ" (CUSTOM) in Claude.ai settings.

### Session 1 Tools (Deployed — 5 build tracking tools)

| Tool | Purpose | Default |
|---|---|---|
| `get_build_summary` | High-level rollup: phase/task/gate counts by status, action cards, drift detection | project=sparkle_suite |
| `get_phases` | Phase list with status, task counts, completion % | project=sparkle_suite |
| `get_tasks` | Filterable by project, phase, status, execution_mode, assignee, overnight_only | project=sparkle_suite |
| `get_gates` | Test gates with pass/fail checklists | project=sparkle_suite |
| `get_action_cards` | Rolling previous/current/next context cards | project=sparkle_suite |

All tools support optional project filter with default (sparkle_suite). Single-build-per-project model.

### Three-Session Build Plan

| Session | Scope | Tools | Status |
|---|---|---|---|
| **1** | Build tracking | get_build_summary, get_phases, get_tasks, get_gates, get_action_cards | ✅ Deployed |
| **2** | Financial + operations | get_financial_snapshot, get_transactions, get_clients, get_maintenance, get_queue, get_expenses | ⏳ Planned |
| **3** | Briefing + projects | get_brief, get_projects, get_ideas | ⏳ Planned |

**Philosophy:** Full dashboard coverage — all data available to Louis on the dashboard should be available to Claude. Credit/token efficiency via prompt discipline (defaults with limits, filters, summary vs detail modes), not tool limitation.

### Integration with Session Open Protocol
Session Open Protocol updated from 3 pulls to 4 pulls (SOP v1.7):
...
4. **Call `get_build_summary()` via NR HQ MCP** — returns phase/task/gate counts, action cards, drift detection
...
### Current Tabs (Post-Restructure — April 14, 2026)

| Tab | Function | Sub-Tabs |
|---|---|---|
...
| Operations | Build pipeline, agent monitoring, sprint status, platform health | Overview · Maintenance · Clients |
| Sales | Client funnel, pipeline, growth milestones | — |
| Ideas | Backlog parking lot | — |
| Build Tracker | Phase progress, task checklists, test gate tracking (becomes Platform Health after SS launch) | Sparkle Suite · NR HQ · VA Compensation |
| Tools | CLI cheat sheets, standing prompt templates, tool references | CLI · Prompts |

**Changes from Phase 2A baseline (cumulative through April 14, 2026):**
- Maintenance tab → Operations sub-tab (commit d01bdfa)
- PA tab → Build Tracker sub-tab as "VA Compensation" (commit 35ca37f)
- Client Directory added as Operations sub-tab (commit 9570b69, then enhanced with contact info + expandable details)
- Tools tab added with combined CLI reference (commits 4784680 + b5bc501)
- Prompts added as Tools sub-tab (not top-level tab) — Session Close prompt baked in
- Queue tab removed — redundant with Build Tracker (April 14, 2026)
- Top nav reduced from original 9 to 7 tabs
...
| MCP | NR HQ MCP Edge Function (nr-hq-mcp) — Claude Chat read access to Supabase |
...
|---|---|---|
| **Phase 1** | Static scaffold, 8 tabs, hardcoded data, deployed | April 2, 2026 |
| **Phase 2A** | 8 Supabase tables, Auth, RLS, dashboard reading from Supabase, CODEBASE_SNAPSHOT updated | April 4, 2026 |
| **Phase 2B** | Stripe + Plaid APIs, daily cron, auto-populated financials, morning/evening briefs, Open Brain → projects pipeline. Architecture locked April 12, code built April 12, infrastructure wiring April 13. | April 13, 2026 |
| **Build Tracker tab** | Phase progress, task checklists, test gate tracking for SS build. 3 Supabase tables, 5 UI components, ~95 tasks seeded. | April 13, 2026 — commits 2d588d4 + 33fac6e |
| **Visual Redesign** | Full NR branding, NRCard hover glow, SVG progress rings, flow map, wave bars, action cards, dark mode only | April 13, 2026 — commit 3db7a10 (37 files) |
| **Build Tracker Enhancements** | Multi-project tabs (SS/NR HQ), rolling action cards, cheat sheet, build_action_log table | April 13, 2026 — commit e4cfd75 (Migration 010) |
| **ProjectTabs Bug Fix** | Ripped out URL routing, replaced with useState. Lesson: simplicity-first. | April 13, 2026 |
| **Dashboard Restructure Tasks 1–6** | Tools tab (CLI + Prompts sub-tabs), Maintenance→Operations, PA→Build Tracker, Client Directory (with expandable details + contact info + click-to-copy), Queue removed | April 14, 2026 — see HQ_Dashboard_Restructure_Plan_v1.2 |
| **CLAUDE.md** | Project-level instructions for Claude Code — NR branding, code rules, lessons learned | April 14, 2026 |
| **Standing Rules v3.8** | Four new rules: No UI without design approval, nothing COMPLETE without Louis verification, restart prompt WHERE ARE WE block, simplicity-first code standard | April 14, 2026 |
| **NR HQ MCP Session 1** | 5 build tracking tools deployed — get_build_summary, get_phases, get_tasks, get_gates, get_action_cards. Claude Chat connector added. | April 14, 2026 |
...
|---|---|---|
| **NR HQ MCP Sessions 2–3** | Session 2: 6 financial + operations tools. Session 3: 3 briefing + project tools. Total: 14 tools for full dashboard coverage. | Prompt writing session |
| **Dashboard Restructure Tasks 7–8** | VA Compensation workflow redesign (Task 7), Client Directory Supabase wiring + auto-update pipeline (Task 8) | Task 7: design session. Task 8: Opus architecture session. |
| **2C** | Gmail agent, agent architecture | Dedicated agent architecture session (standing rule) |
| **3** | Customer Board, Lifecycle Workflow Map, Onboarding Funnel, Automation Health, Thumper CEO View, BP Intelligence, Project Financials, Agent Monitoring Panel, Research Intelligence Layer, Data Storage Center, Built-In Chatbot, Voice Briefing | All sections complete ✅ |
...
6. ✅ Build Tracker tab — scoped, built, seeded, live → HQ_Master_Plan_v1.6
7. ✅ Dashboard visual redesign + Build Tracker enhancements + restructure Tasks 1–4 → HQ_Master_Plan_v1.7
8. ✅ NR HQ MCP architecture + deployment (Session 1) + restructure Tasks 5–6 + Queue removal → HQ_Master_Plan_v1.8
9. ⏳ Dashboard restructure Tasks 7–8 (VA workflow redesign, Client Directory wiring)
10. ⏳ NR HQ MCP Sessions 2–3 (financial + operations + briefing tools)
...
| OD-15 | ~~Claude Chat Supabase read access~~ | ~~Opus architecture session~~ | ✅ Resolved — NR HQ MCP deployed (Option B: separate Edge Function, greenfield, service role + MCP_ACCESS_KEY gate). Session 1 live with 5 tools. Sessions 2–3 pending for full coverage. (April 14) |
...
| Apr 13 | Build Tracker tab scoped — 4 UI sections, 3 Supabase tables, all data from SS Master Build Plan |
| Apr 13 | Build Tracker tab BUILT AND DEPLOYED — commits 2d588d4 + 33fac6e, ~95 tasks seeded, live at /build-tracker |
| Apr 13 | Build Tracker uses manual data entry initially, future auto-updates via CODEBASE_SNAPSHOT parsing |
| Apr 14 | Standing Rules v3.8 locked — four new rules (17–20): No UI without design approval, nothing COMPLETE without Louis verification, restart prompt WHERE ARE WE block, simplicity-first code standard |
| Apr 14 | Dashboard restructure: Maintenance→Operations sub-tab, PA→Build Tracker as VA Compensation, Client Directory added under Operations, Tools tab with combined CLI reference |
| Apr 14 | CLAUDE.md created for neon-rabbit-hq repo — lessons learned permanently encoded for Claude Code |
| Apr 14 | VA Compensation workflow redesign needed — future session for auto-update pipeline |
| Apr 14 | Client Directory needs Supabase wiring + auto-update — part of Claude Chat ↔ Supabase architecture |
| Apr 14 | Claude Chat Supabase read access (Option D) — approved, Opus architecture session completed |
| Apr 14 | Gemini worth testing for frontend/UX builds — future comparison against Claude Code |
| Apr 14 | NR HQ MCP deployed — Option B (separate Edge Function), greenfield, service role + MCP_ACCESS_KEY gate, 5 build tracking tools, full dashboard coverage philosophy |
| Apr 14 | OD-15 resolved: Claude Chat Supabase read access live via NR HQ MCP |
| Apr 14 | Session Open Protocol updated from 3 pulls to 4 pulls (added get_build_summary) |
| Apr 14 | Queue tab removed — redundant with Build Tracker |
| Apr 14 | Prompts moved to Tools sub-tab (not top-level tab) — Session Close prompt as first entry |
| Apr 14 | Session Close prompt locked — 4-part protocol with forced stops and verification checklists |
| Apr 14 | Prompts earn their way in through repeated use — no speculative brainstorming |
| Apr 14 | Client Directory enhanced with real contact info, expandable details (website, socials, timezone, rate, team), click-to-copy on codes/phone/email |
| Apr 14 | Top nav finalized at 7 tabs: Pulse, Financial, Operations, Sales, Ideas, Build Tracker, Tools |
| Apr 14 | HQ_Dashboard_Restructure_Plan_v1.2 generated — Tasks 1–6 complete, Tasks 7–8 pending |
