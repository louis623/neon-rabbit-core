...
**Version:** 1.3 | **Created:** April 10, 2026 | **Last Updated:** April 12, 2026 | **Status:** ACTIVE — Extension built, tested, and deployed to Lindsey. Website component live on milehighfizz.com. Remaining 4 clients ready for deployment.
...
### Queue Building Logic (Inverted List, No Dedup)
...
6. **No deduplication or collapsing of any kind** — every unrevealed order gets its own entry, even if the same name appears multiple times in a row. This ensures the queue visibly shrinks by one entry when any single order is revealed.
7. Result: `queue[0]` = "Currently Unboxing", `queue[1]` = "On Deck", etc.

**Why no collapse:** Consecutive collapse (removed in commit ba1e5a6) caused a visible bug — when a customer with multiple orders had one revealed, the queue didn't visibly change because the remaining duplicates still collapsed to one entry. Customers watching the show saw no movement. Each order is a separate unboxing moment and must be its own queue entry.

### Known Bug — Individual Order Tracking (FIXED — commit ba1e5a6)

Previously, the queue used consecutive duplicate collapsing which caused a visible bug: revealing one order for a multi-order customer didn't visibly change the queue. Fixed by removing all dedup/collapse logic. Each unrevealed order now gets its own queue entry. The queue visibly shrinks by one when any order is revealed.

Note: The queue is still name-based, not order-based. When the full Sparkle Suite platform is built, individual order tracking by OrderID will provide even more precise queue management. But the visible behavior is now correct.
...
6. Sends the full list — no dedup or collapsing of any kind
...
### Bug: Queue showed each name only once (full dedup) — FIXED
...
**Fix (commit 07a61f3):** Removed full dedup. Replaced with consecutive-collapse filter.

**Subsequent fix (commit ba1e5a6):** Removed consecutive collapse entirely. The collapse itself caused a visible bug — revealing one order for a multi-order customer didn't visibly change the queue because remaining duplicates still collapsed. Now every unrevealed order gets its own entry with no filtering of any kind.
...
- [x] Every unrevealed order gets its own entry (no dedup, no collapsing)
- [x] Same name appearing multiple times displays correctly
...
- Individual order tracking by OrderID for more precise queue management (current name-based approach works correctly but OrderID tracking enables richer features like order-specific status)
...
6. No dedup or collapsing — every unrevealed order is its own queue entry
