...
**Version:** 3.5 | **Created:** April 9, 2026 | **Last Updated:** May 5, 2026 | **Status:** ACTIVE

**v3.5 CHANGES (May 5, 2026):**
1. **TRADE BOARD MVP RULES CLARIFIED FROM REP PHONE CALL.** Current Sparkle Suite trade board scope is item-for-item only. No pay-the-difference path, no credit path.
2. **MATCHING BASIS LOCKED FOR NOW.** Trades are compared by same collection + same jewelry type, not by Bomb Party MSRP. Birthday pieces may trade across months as long as they stay inside Birthday and the same jewelry type.
3. **MSRP DEMOTED TO REFERENCE DATA.** MSRP can still display on listings and in the jewelry database, but it is not the trade-parity engine and should not drive approval logic, request inbox comparisons, or primary MVP filtering.
4. **TASK 2.5 UNBLOCKING IMPLICATION.** This clarification materially affects Phase 3 logic and later dashboard assumptions, but it is sufficient to remove the old "waiting on trade-board scope truth" blocker for the Phase 2 Trade Board page shell.

**v3.4 CHANGES (May 4, 2026):**
1. **PHASE 2 EXECUTION MODEL CLARIFIED.** Sparkle Suite still has multiple planned kit families (Amethyst, Garnet, Velvet, Rose Quartz, Amber), but only **Amethyst** is being built through the full system right now. The other kits stay deferred until Amethyst is proven end-to-end across the real platform.
2. **PHASE 2 LANGUAGE TIGHTENED.** Replaced ambiguous "single template" shorthand with **Amethyst-first execution** so the plan no longer reads like the long-term multi-kit product vision was dropped.
3. **TASK ORDER ALIGNED TO HQ TRACKER.** Phase 2 now matches the live HQ ordering: `2.6 = Join Team`, `2.7 = Amethyst-first appearance hooks / future kit scaffolding`, `2.8 = Custom domain routing`, `2.9 = Responsive/mobile pass`, `2.10 = Template variable system`.
4. **TASK 2.7 REFRAMED.** This task is now the minimum architecture needed to avoid backtracking later, not a broad "design more themes now" task. Do not spend current build time polishing other kits before Amethyst is proven.
5. **TEST GATE 2 UPDATED.** Theme validation now checks that any future-kit hooks do not break Amethyst, rather than requiring broad multi-theme polish before prototype validation.

**v3.3 CHANGES (May 1, 2026):**
1. **PHASE 2 RESTRUCTURED.** Site architecture simplified from 4 pages to 3 pages + 1 external link based on May 1, 2026 design session wireframing (5 iterations). Changes:
   - **Task 2.1 scope narrowed:** ONE template (not 4-5). Curated color palette (8-12 presets) added. CSS custom property architecture for theming.
   - **Task 2.4 (Homepage) restructured:** About section added (bio + 3 media slots + social links — replaces eliminated About page). CTA buttons in hero ("Browse my trade board" + "Watch me live"). Trade board preview REMOVED. "What is Bomb Party?" card + TikTok embed REMOVED (moved to universal info site).
   - **Task 2.5 (About page) ELIMINATED.** Content absorbed into homepage section.
   - **Task 2.7 (U&D/FAQ page) ELIMINATED from per-rep build.** Content moves to a separately scoped universal info site (yoursparklesuite.com/learn or subdomain). Rep sites link to it via nav.
   - **NEW Task 2.5 (Trade Board page shell).** Full page in nav. Desktop: LRQ left 1/3, trade board right 2/3, LRQ collapses when no show. Mobile: trade board full width + collapsible LRQ drawer banner. Content filled by Phase 3 (trade board) and Phase 10 (LRQ).
   - **NEW Task 2.6 (Curated color palette / theme system).** 8-12 presets, CSS custom property switching, color_scheme field on site_settings, new Thumper tool.
   - **Task 2.7 renumbered to 2.8 (Custom domain routing).** Updated to account for Trade Board as named page route.
   - **Task 2.8 renumbered to 2.9 (Responsive/mobile pass).** Must include Trade Board page mobile layout (LRQ collapsible drawer).
   - **Task 2.9 renumbered to 2.10 (Template variable system).** Unchanged.
2. **PHASE 2 HEADER UPDATED.** "4 Pages + Design System" → "3 Pages + Design System + External Link."
3. **TEST GATE 2 UPDATED.** About page and U&D/FAQ page checks removed. Trade Board page shell check added. Color theme switching check added. Page count references updated.
4. **CRITICAL PATH UPDATED.** Phase 2 now shows "3 rep pages + external link" instead of "4 pages."
5. **PARALLEL EXECUTION MAP UPDATED.** Wave 3 references updated to match new page structure.
6. **OVERNIGHT CANDIDATES UPDATED.** Task 2.7 (U&D/FAQ, formerly strong overnight candidate) removed. Task 2.5 (About page) removed.
7. **NEW SEPARATE SCOPE: Universal Info Site.** "What is Bomb Party?" explainer, 3 value tier cards, 36 FAQs across 5 categories, rep referral link-back. Hosted on yoursparklesuite.com, NR-maintained. NOT Phase 2 — separate build task. Documented in Phase 2 notes and Parking Lot.
8. **PHASE 9 REFERENCES UPDATED.** "Pages 3 & 4" → "Join Team page" where applicable. ISR/SSG strategy updated for 3-page site.
9. **GREY AREA UPDATED.** "Branding menu design" now references curated color palette rather than template selection.
10. **THUMPER DESIGN/COPYWRITING EXPERT noted** as system prompt enhancement (Task 1.2 scope or later pass) — NOT a Phase 2 build task.
...
**PRD-first workflow (v3.20 Standing Rules — April 30, 2026):** Codex is the primary builder. Chat writes PRDs → Codex builds → Chat reviews → Claude Code cross-checks when needed → Louis verifies (Rule 18). Single-task prompts for Claude Code are the fallback when a PRD is overkill or the task requires real-time repo interaction.
...
**Trade type:** One-for-one trades only. No purchases. No cash transactions at launch. No pay-the-difference flow. No credit flow. For now, a trade is only eligible when it stays within the same collection and the same jewelry type. Bomb Party MSRP may display for reference, but it is not the rule that determines whether a trade is acceptable.
...
**Amethyst-first template execution (clarified May 4, 2026):** Sparkle Suite still has multiple planned customer-facing kit families, but the build sequence does not. Amethyst is the only kit being carried through the full system right now. There is no template picker/menu at launch. Reps start on Amethyst, and the rest of the stack gets proven there before meaningful work begins on Garnet, Velvet, Rose Quartz, or Amber.

**Rep personalization inside Amethyst (locked May 1, 2026; clarified May 4, 2026):** While Amethyst is the active build target, reps can still personalize the parts that matter operationally through Thumper: hero image, tagline, banner text, ticker, streaming links, social handles, bio text, media slots, and any narrow appearance hooks we intentionally preserve to avoid future rework. The reps are the variety. Amethyst is the testbed.
...
**Authoritative Phase 2 task map (May 4, 2026):** Use this as the source of truth for Phase 2 intent and numbering. It supersedes any stale wording in the legacy rows below.

| # | Current meaning |
|---|-----------------|
| 2.1 | Amethyst design system lock |
| 2.2 | Global header |
| 2.3 | Global footer |
| 2.4 | Build Amethyst homepage from locked design files |
| 2.5 | Trade Board page shell |
| 2.6 | Join Team page |
| 2.7 | Amethyst-first appearance hooks / future kit scaffolding only |
| 2.8 | Custom domain routing |
| 2.9 | Responsive / mobile-first pass |
| 2.10 | Template variable system |

**Important reading rule:** `2.7` is **not** "go design more templates now." It means preserve only the minimum Amethyst-safe appearance, token, or site-settings hooks that prevent future backtracking. Do not spend current build time on extra kits, template-picker UX, or broad palette polish before Amethyst is proven through the rest of Sparkle Suite.
...
| 1.2 | Thumper system prompt — refinement pass | 🔧 Medium | No — needs Louis review | Opus session in Claude Chat. Task 1.1 ships a first-draft system prompt. Task 1.2 refines: persona tuning, scope lock (10 allowed domains) verification, warm redirect script tightening, trade board AI notice review, non-affiliation disclaimer verification, content screening rule tuning, personality calibration (concise/warm/plain language). Only fires after 1.1 ships and real rep-ish conversations expose prompt weaknesses. **Also scoped for Thumper design/copywriting expert enhancement** (May 1, 2026 design session): Thumper becomes an expert at guiding reps to quality content — drafting bio text, suggesting color combinations, coaching on banner/ticker copy, warning about poor choices. System prompt content addition. |
...
| 1.5 | Thumper trade board tools | 🔧 Medium | 🌙 Yes | Implement 10 tools. All call the shared service layer (Task 0.1b). Tools: (1) add_listing — flexible input (item number OR label photo OR both), single + batch mode, three entry points, canonical photo fallback, (2) get_my_board — rep's board with filters/sorting/summary stats, (3) remove_listing — soft-remove with pending request auto-cancel warning, (4) get_trade_requests — incoming request inbox with collection/type compatibility context, not MSRP parity logic, (5) approve_trade — creates trade_fulfillment record, auto-cancels other pending requests for same listing, (6) reject_trade — piece reappears on board, (7) search_jewelry_database — full catalog search with is_on_my_board flag and aggregate stats only (no rep identification), (8) update_listing — partial update of trade prefs/photo/notes, (9) get_trade_history — completed/denied/cancelled with summary analytics, (10) update_fulfillment_status — 3-status pipeline (approved → shipped → completed) with add_to_board option on completion. |
| 1.6 | Thumper calendar tools | ⚡ Quick | 🌙 Yes | add_show, list_my_shows, update_show, cancel_show. Recurring shows (daily/weekly, 1mo/3mo/ongoing with 6-month cap). Multi-discount-code support (JSONB array, max 10). |
| 1.7 | Thumper site customization tools | ⚡ Quick | 🌙 Yes | update_banner_text, update_streaming_links, update_site_setting (covers tagline, ticker, hero image, social handles — cross-table write to reps.social_handles). |
| 1.8 | Thumper SMS/email tools | ⚡ Quick | 🌙 Yes — after Phase 5 | send_sms_notification, send_email_notification, get_notification_preferences. Stubs returning "coming soon" until Phase 5 wires providers. |
| 1.9 | Thumper memory — rep notes table | ⚡ Quick | 🌙 Yes | write_rep_note, read_recent_rep_notes. End-of-conversation summary write. Next conversation loads recent notes. Simple chronological text — no vector search at launch. |
| 1.10 | Thumper model routing | ⚡ Quick | 🌙 Yes | Haiku 4.5 default. Deferred to Phase 11 (pre-launch integration testing) — Haiku handles all 21 tools without issues. Sonnet 4.6 escalation evaluated under live conditions with full tool surface. |
...
## Phase 2 — Site Template (3 Pages + Design System + External Link)

**What this builds:** The rep-facing website — 3 pages (Homepage, Trade Board, Join Team), global header/footer, design system, single template with curated color palette, custom domain routing. Plus an external "Learn about Bomb Party" link in nav pointing to a separately built universal info site.
...
**Estimated time:** 🏗 Large — 4–6 sessions over 2–4 days

**Design decisions locked May 1, 2026 and clarified May 4, 2026:**
- Sparkle Suite still supports multiple kit families long-term, but only **Amethyst** is being built through the full system right now.
- No template picker/menu at launch. All reps start on Amethyst until the rest of the system is proven.
- Do not spend current build time designing or polishing other kits unless a tiny architectural hook prevents backtracking later.
- About page eliminated — bio section on homepage instead.
- U&D/FAQ page eliminated from per-rep build — moves to universal info site (separate scope).
- Trade Board page combines trade board content (Phase 3) + Live Reveal Queue slot (Phase 10).
- Seasonal/holiday skins concept PARKED for future, alongside the later Garnet/Velvet/Rose Quartz/Amber rollout.

### Locked vs Customizable Reference

**LOCKED (NR controls, push-update):** Page structure/layout, typography, heading hierarchy, footer content and disclaimers, nav bar structure (rep name auto-injected), "Never miss a show!" signup form, Join Team benefit cards and FAQ questions (answers customizable), CTA button text and behavior, brand separation disclaimers.

**REP-CUSTOMIZABLE (via Thumper):** Banner text, ticker text, tagline, hero image, streaming links, social handles, bio text (about section on homepage), 3 media slots (photo or video embed), color scheme (curated palette — 8-12 presets), Join Team page visibility (hide from nav), Join Team FAQ answers.

### Tasks

| # | Task | Time | Can Run Overnight | Notes |
|---|------|------|-------------------|-------|
| 2.1 | Design system — single template, fonts, colors, button/card standards, curated palette | 🔧 Medium | No — needs Louis review | Opus session in Claude Chat. Design ONE template. Define curated color palette (8-12 presets). CSS custom property architecture for theme switching. Gradient buttons, neon glow cards, hover/tap animations. Must look 2028, not 2010. Frontend Design skill REQUIRED. |
| 2.2 | Global header — banner + ticker + nav | 🔧 Medium | 🌙 Yes | Sticky. Dismissible banner, scrolling ticker, black header bar. Nav items: Home, Trade Board, Join Team + "Learn about Bomb Party ↗" (external link, new tab to universal info site). Rep name auto-injected. Data from Supabase (rep controls via Thumper). |
...
| 2.4 | Homepage — hero + about section + signup + show cards | 🏗 Large | Partial | Hero with animation, streaming buttons. CTA buttons ("Browse my trade board" → Trade Board page, "Watch me live" → streaming platform). **About section:** 2-3 paragraphs bio text + 3 media slots (photo or video/TikTok embed) + social links row — all rep-customizable via Thumper. "Never Miss a Show!" signup form (unchecked consent boxes). Next 2 shows countdown cards. |
| 2.5 | Trade Board page shell — layout + nav routing | 🔧 Medium | 🌙 Yes | Full page in site nav. **Desktop, show active:** LRQ left 1/3 column, trade board right 2/3. **Desktop, no show:** LRQ column collapses, trade board full width. **Mobile, show active:** Trade board full width + "LIVE NOW — X items revealed" banner at top, tap-to-expand collapsible drawer showing LRQ. **Mobile, no show:** Trade board full width, no banner. Phase 2 builds the page shell + nav routing + LRQ slot. Phase 3 fills trade board content. Phase 10 fills LRQ content. Shell should assume trade-only UX — no buy-now, pay-difference, or credit surfaces. |
| 2.6 | Curated color palette / theme system | 🔧 Medium | Partial | Design 8-12 color presets. Implement CSS custom property switching. Add `color_scheme` field to `site_settings` table (migration). New Thumper tool: `update_color_scheme` (or fold into existing `update_site_setting`). Each preset defines primary, secondary, accent, background, text colors. All site components read from CSS custom properties. |
| 2.7 | Join Team page — grid + benefits + FAQ | 🔧 Medium | 🌙 Yes | Team member grid (scaling cards). Six locked benefit cards. Six locked FAQ questions with customizable answers. Final CTA. Hidden from nav if rep opts out. |
| 2.8 | Custom domain routing | 🔧 Medium | 🌙 Yes | Host-based detection in Next.js. Custom domains forward to rep's location within deployment. Dynamic data loading per hostname. Must account for Trade Board as a named page route. |
| 2.9 | Responsive / mobile-first pass | 🔧 Medium | No — needs Louis phone testing | Every page tested on mobile. Cards stack vertically. Hamburger menu. Touch targets. Scrollable pill filters. **Trade Board page mobile layout** must be verified: LRQ collapsible drawer behavior, "LIVE NOW" banner, trade board full-width stability. |
...
**Phase 2 clarification (May 4, 2026):** The live HQ tracker is the source of truth for current Phase 2 ordering and status. In tracker terms, `2.6 = Join Team complete` and `2.7 = still-open future-looking appearance/platform scaffolding work`. Read any remaining "color palette/theme system" language here narrowly: preserve only the minimum Amethyst-safe hooks that prevent future rework. Do **not** treat that as permission to spend build time on extra kits, broad palette exploration, or template-polish work before Amethyst is proven through the rest of Sparkle Suite.

### Test Gate 2

Before Lindsey prototype:
- [ ] All 3 pages render correctly on desktop and mobile (Homepage, Trade Board, Join Team)
...
- [ ] Nav shows correct items: Home, Trade Board, Join Team + "Learn about Bomb Party ↗" (external link)
- [ ] Footer disclaimers display correctly
- [ ] "Never Miss a Show!" form submits to Supabase
- [ ] About section on homepage renders bio text + 3 media slots + social links
- [ ] Trade Board page shell renders with correct desktop 2-column and mobile collapsible drawer layout
- [ ] Color theme switching works — changing `color_scheme` in site_settings updates all CSS custom properties
- [ ] Custom domain routing resolves correctly
- [ ] At least one color theme looks polished and professional (Louis approval)
...
| # | Task | Time | Can Run Overnight | Notes |
|---|------|------|-------------------|-------|
| 3.1 | Trade board listing display | 🔧 Medium | 🌙 Yes | Grid/list view of rep's available pieces. Each listing shows: photo, collection, type, material, MSRP, rarity tags. Brand separation label per listing. MSRP is display/reference only — not a parity signal. |
| 3.2 | Filtering system | 🔧 Medium | 🌙 Yes | Collection (scrollable/searchable), jewelry type, size where applicable, material, rarity tags (unicorn, diamond). Mobile: horizontal pill bars for primary filters, expandable panel for secondary. Extensible by design. |
...
| 3.5 | Thumper trade notification + approve/deny | 🔧 Medium | 🌙 Yes | Real-time notification in Thumper conversation. Rep can ask follow-up questions about their board. Approval context should emphasize same-collection + same-type matching, with MSRP available only as reference. Approve = piece gone permanently. Deny = piece reappears. Quick-action buttons. |
| 3.6 | Clickwrap at point of listing | ⚡ Quick | 🌙 Yes | Rep certifies ownership, listing accuracy, and that trade-board decisions are ultimately rep-controlled. MSRP accuracy can still be certified as catalog/reference data, but MSRP is not the trade-parity engine. Cannot proceed without checking. |
...
**Depends on:** Test Gates 0, 1, 3, and 4 passing. Phase 2 needs at least homepage + header/footer + Trade Board page shell working.
...
- Homepage with show cards (calendar) and about section
...
- Onboarding pipeline
- Universal info site (standalone build)
...
| 6.2 | Trade board home view (full management) | 🔧 Medium | 🌙 Yes | NOT read-only — full management controls. Summary bar (total pieces, pending request badge, recent trade activity). Listing grid with photos, details, quick-action buttons (edit, remove, view requests). Filter bar: collection, type, material, size where relevant. Sort toggle. "Add to Board" button (single + bulk). All actions call shared service layer. |
| 6.3 | Trade request inbox (slide-out panel or tab) | 🔧 Medium | 🌙 Yes | Within trade board view — rep never leaves board context. Pending requests with customer name, description, and collection/type fit context. MSRP may display as reference but not as the primary approval comparison. Prominent Approve and Deny buttons per request. Notification badge count. |
...
- [ ] Jewelry Library search works and "Add to My Board" creates correct listing
- [ ] Messages section receives and displays NR announcements
- [ ] Calendar shows upcoming shows
- [ ] SMS wallet balance displays correctly
- [ ] Site analytics show real data (PostHog)
- [ ] Account page shows subscription status and allows self-service cancellation
- [ ] Thumper accessible from every dashboard page

---

## Phase 7 — AI Photo Enhancement Pipeline

**What this builds:** Automated photo processing via Photoroom API — background removal, quality enhancement, and QA screening.

**Depends on:** Phase 1 (Thumper camera integration), Phase 3 (pieces have photos)

**Parallel tracks:** 🟢 PARALLEL with Phases 5, 6, 9
...
| # | Task | Time | Can Run Overnight | Notes |
|---|------|------|-------------------|-------|
| 7.1 | Photoroom API integration | 🔧 Medium | 🌙 Yes | Background removal + enhancement. Output stored in Supabase Storage. Fallback: if Photoroom fails, keep original photo. |
| 7.2 | Photo QA scoring agent | 🔧 Medium | 🌙 Yes | Vision model evaluates output quality. Scores: lighting, clarity, centering, background removal quality. Flags low-quality results for manual review. |
| 7.3 | Bulk processing pipeline | 🔧 Medium | 🌙 Yes | Queue system for batch uploads. Process overnight. Results ready for rep review next session. |
| 7.4 | Photography kit standardization | 🔧 Medium | Needs DUCLUS results | Define standard kit. Lightbox validation. Camera recommendations. Three-tier pricing model. |

### Test Gate 7

- [ ] Photo submitted through Thumper gets background removed
...
| 8.20 | Branding menu | 🔧 Medium | Needs design session | Menu of options rep selects from during onboarding. Curated color palette selection, hero image direction. Details TBD — informed by Phase 2 design system (Task 2.1 + 2.6). |
...
| 9.4 | Localized content wrappers (Join Team page) | ⚡ Quick | 🌙 Yes | Rep name + city injected into Join Team page. Provides domain uniqueness for crawlers. |
| 9.5 | Canonical tags per custom domain | ⚡ Quick | 🌙 Yes | Independent canonical tags. No cross-domain canonical issues. |
| 9.6 | ISR/SSG/SSR rendering strategy | 🔧 Medium | 🌙 Yes | ISR for homepage (trade board/calendar freshness). SSG for Join Team page (locked content). SSR for dashboard only (not indexed). Trade Board page: ISR with short revalidation (listings change frequently). |
...
- [ ] Localized wrappers make Join Team page unique per rep
...
- **Branding menu design** (Phase 8.20) — Curated color palette selection and hero image direction during onboarding. Informed by Phase 2 design system (Tasks 2.1 + 2.6). Design session item.
...
- **Seasonal/holiday skins** — NR drops themed skins (Christmas, Valentine's, etc.) reps swap in via Thumper. Same CSS custom property system as curated color palette, different preset values. Future engagement/delight feature.

### Universal Info Site (Separate Scope — Not Phase 2)

- **"What is Bomb Party?" explainer + value tier cards + FAQ** — Hosted on yoursparklesuite.com/learn (or subdomain). 3 value tier cards (Unicorn/Diamond/Standard). 36 FAQ questions across 5 accordion categories. All content NR-maintained, push-update. Rep sites link to it from nav ("Learn about Bomb Party ↗" opens new tab). Rep referral info passed in URL so universal page shows "You came from [Rep Name]'s site" + link back. One-time build, separate from Phase 2. Build when Phase 2 site template is close to done so the external link has somewhere to go.
...
Session B: Phase 2.4–2.7 — Site pages (Homepage, Trade Board shell, Join Team)
...
- Phase 2.5 — Trade Board page shell (layout spec locked)
...
| Phase 2: Site Template | 4–6 | 2–4 | Phase 1, 10 |
...
| **TOTAL** | **~37–55 sessions** | **~22–34 calendar days** | |
