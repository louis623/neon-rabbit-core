...
Covers show/event management — adding, listing, updating, and cancelling shows on a rep's calendar. Supports recurring show series and multi-code discounts.

---

#### `addShow`

**Purpose:** Add a single show or spawn a recurring series to the rep's calendar.
...
**Behavior:**
- Single show: inserts one `calendar_events` row with status "scheduled"
- Recurring show: spawns ALL instances up front as individual rows sharing the same `recurrence_group_id` UUID. No cron job, no background function.
  - Daily cadence: every day from `eventTime` for the specified duration
  - Weekly cadence: same day of week from `eventTime` for the specified duration
  - Duration caps: 1 month (~30 daily / 4 weekly), 3 months (~90 daily / 13 weekly), ongoing = 6-month cap (180 daily / 26 weekly)
- `discountCodes` stored as JSONB array of `{code, description}` pairs. Max 10 enforced at service layer.
- `featuredCollections` stored as TEXT[] — collection names, not FKs
- Discount codes inherit from parent to all spawned recurring instances
- `is_recurring = true` and `recurrence_rule` set on all rows in a series

**Validation:**
- `platform` is required
- `eventTime` is required and must be a valid future timestamp
- `discountCodes` max 10 items → error if exceeded
- `recurring.cadence` must be 'daily' or 'weekly'
- `recurring.duration` must be '1_month', '3_months', or 'ongoing'
...
**Purpose:** Return the rep's upcoming and recent shows.
...
**Behavior:**
- Pure read operation
- Default sort: `event_time` ascending (next upcoming show first)
- Default filter: scheduled and live shows (excludes cancelled/completed unless filtered)
...
**Purpose:** Modify an existing show's details. Supports bulk series updates via `applyToSeries`.
...
**Behavior:**
- Partial update — only provided fields change
- If `applyToSeries = true` AND the show has a `recurrence_group_id`: updates all FUTURE shows in the series sharing the same `recurrence_group_id` (past/completed shows are not modified). `eventTime` is NOT applied to series — each show keeps its own time. Other fields (title, platform, discountCodes, etc.) apply to all future instances.
- If `applyToSeries = false` or omitted: updates only the specified show

**Validation:**
- Show must exist and belong to `repId`
- Show must be in "scheduled" status (can't edit completed/cancelled shows)
- `discountCodes` max 10 items if provided

**Return:**
```typescript
interface UpdateShowResult {
  showId: string;
  updatedFields: string[];
...
**Purpose:** Cancel a single scheduled show.
...
- Per-show only — no bulk series cancellation at this time
- Does NOT delete the row (soft cancel for audit trail)

**Validation:**
- Show must exist and belong to `repId`
- Show must be in "scheduled" status → error if already cancelled/completed

**Return:**
```typescript
interface CancelShowResult {
  showId: string;
  title?: string;
  platform: string;
  eventTime: string;
...
As of Task 1.6B ship (May 1, 2026), 13 Thumper tools map to these service functions:
...
| `add_show` | `addShow` | calendar.ts | auth | No |
| `list_my_shows` | `listMyShows` | calendar.ts | auth | No |
...
*This spec is the single source of truth for the Sparkle Suite shared service layer. Claude Code reads this when building Phase 1 Thumper tools, Phase 3 (Trade Board UI), Phase 4 (Calendar UI), and Phase 6 (Rep Dashboard). Update it when business rules change. Do not update for features still in brainstorming — those go to Open Brain.*
