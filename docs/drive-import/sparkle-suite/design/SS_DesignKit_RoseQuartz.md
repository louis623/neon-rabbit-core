...
URL: https://www.fontpair.co/playground/bitter-archivo?color=E879F9&style=soft-marketing&icons=1&iconLib=eva

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
TYPOGRAPHY RULES:
→ Use Sharpie for ALL headings (h1–h4), display text, and hero copy
→ Use Ranade for ALL body text, captions, labels, and UI copy
→ Do NOT substitute with Inter, system-ui, Georgia, or any fallback font
→ Load via Google Fonts: https://fonts.googleapis.com/css2?family=Sharpie:wght@400;500;600&family=Ranade:wght@400;500&display=swap
...
→ Foreground: #2D0036 — use for all primary text, headings, and icons
→ Foreground Muted: #49384C — use for secondary text, placeholders, and metadata
→ Primary: #E879F9 — use for CTAs, active states, links, and key UI accents
→ Accent: #63146E — use for hover states, pressed buttons, and deep emphasis
→ Border: #FFB0FF — use for all dividers, input borders, and card outlines
→ Do NOT introduce colors outside this palette (no Tailwind defaults like blue-500 or gray-400)
→ Do NOT use black (#000000) — use #2D0036 as the darkest value
...
COMPONENT RULES:
→ Every button must use Primary (#E879F9) background with white text, rounded-md
→ Every card must use Border (#FFB0FF) outline, white background, consistent padding (16–24px)
→ Every input must use Border (#FFB0FF) outline, Foreground Muted placeholder text
→ Every link must use Primary (#E879F9), Accent (#63146E) on hover — no underline by default

FRAMEWORK RULES:
→ Use shadcn/ui components with Tailwind CSS
→ Map semantic tokens: primary → #E879F9, foreground → #2D0036, muted → #49384C, border → #FFB0FF
→ Apply fonts via CSS variables: --font-heading: 'Sharpie', --font-body: 'Ranade'
