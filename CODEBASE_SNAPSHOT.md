...
## Session 2026-04-30 — SS Phase 1 Task 1.5D (search_jewelry_database, update_listing, get_trade_history tools + system prompt update)

**Goal:** Round out the Thumper trade-board surface with the three remaining tools the Task 1.5A service layer already supports but no Thumper handler exposed: `search_jewelry_database` (catalog free-text search), `update_listing` (patch editable listing fields), and `get_trade_history` (past trades + analytics). Update the system prompt for the new tool count + capability surface.

**New files:**
- `lib/thumper/tools/search-jewelry-database.ts` — `searchJewelryDatabaseTool: ToolDefinition` (`readOnly: true`). Inputs: `query` (string) + optional `limit` (1-50). Service-role admin client (cross-rep COUNT for `activeListingsCount` aggregation requires admin per `lib/services/jewelry-database.ts:7-11`); `ctx.repId` still flows through so the per-rep `isOnMyBoard` flag remains accurate. Flattens `JewelryDatabaseResult[]` into `{ count, results:[{designId, itemNumber, designName, material, mainStone, msrp, photoUrl (canonicalPhotoUrl), type (typePrefix), collectionName, isOnMyBoard, activeListingsCount}] }`. Translates `ServiceError → ThumperToolError`. NO filter parameters beyond `query` + `limit` — the underlying service does NOT accept `itemNumber`/`collectionFilter`/`typeFilter`/`materialFilter`/`msrpMin/Max`/`stoneFilter`/`sortBy`, so the schema mirrors that surface exactly.
- `lib/thumper/tools/update-listing.ts` — `updateListingTool: ToolDefinition` (`readOnly: false`, **no** `needsApproval` — reversible re-patch). Uses `ctx.supabase` auth client (rep-scoped UPDATE on `trade_listings` per service header `lib/services/trade-board.ts:20`). Editable surface is exactly the four fields in `UpdateListingInput` (`lib/services/types.ts:121-127`): `repNotes`, `tradePreferences`, `listingPhotoUrl`, `useCanonicalPhoto`. MSRP, design name, material, item number, and other catalog/design data are NOT editable — that data is shared across reps. **Two `.refine()` schema checks plus matching handler defense-in-depth:** (1) at-least-one-patch-field (the service silently accepts an empty patch and bumps `updated_at` per `lib/services/trade-board.ts:465`, which would create a no-op mutation + misleading `listing_updated` audit row); (2) reject `listingPhotoUrl` + `useCanonicalPhoto:true` simultaneously (service gives `useCanonicalPhoto` precedence at `lib/services/trade-board.ts:468` and silently ignores `listingPhotoUrl`, so accepting both would lead to misleading `patchedFields` audit). Allowed edge: `listingPhotoUrl: null` + `useCanonicalPhoto: true` (both express "clear the rep photo"). Audit: `actionType: 'listing_updated'`, `afterState.patchedFields: string[]` mirrors the keys the service actually applied (when `useCanonicalPhoto:true` with non-null URL is somehow allowed through, `listingPhotoUrl` is excluded from `patchedFields` so audit reflects service behavior, not input). Audit isolation matches `remove-listing.ts` (audit failure → console + best-effort `logIncident('audit_write_failed','warn')` → mutation result still returned). `inputSchema` is exported for direct schema-level testing.
- `lib/thumper/tools/get-trade-history.ts` — `getTradeHistoryTool: ToolDefinition` (`readOnly: true`). Uses `ctx.supabase` auth client (pure rep-scoped read per `lib/services/trade-requests.ts:13`). Inputs: `limit` (optional, 1-100). The service hardcodes `status IN ('approved','denied')` internally — there is NO `statusFilter` input here. Pending requests live in `get_trade_requests`, not this tool. Flattens `TradeHistoryItem[]` into `{ count, items:[{requestId, listingId, customerName, status, fulfillmentStatus, createdAt, completedAt, fulfillmentDays, design:{itemNumber, designName, msrp, type, collectionName}}], summary:{totalCompleted, totalMsrpTraded, avgFulfillmentDays, topDesign, repeatCustomers} }`. Translates `ServiceError → ThumperToolError`.
- `tests/thumper/jewelry-database.test.ts` — vitest unit tests for `search_jewelry_database` (6 cases): admin-client + repId + `{query,limit}` threading, repId-still-flows-through guard for `isOnMyBoard`, empty-result passthrough, limit-forwarded vs omitted, `ServiceError → ThumperToolError` translation, registry `readOnly:true` + `name` assertion via the exported `searchJewelryDatabaseTool` (NOT via `make*Tool()` factory return value — `readOnly` lives on the `ToolDefinition`, not the built tool instance, per `lib/thumper/tools/types.ts:15`).
- `tests/thumper/trade-board-tools.test.ts` — vitest unit tests for `update_listing` (13 cases). Three describe blocks: (a) **schema** via direct `inputSchema.safeParse()` — empty patch rejected, missing listingId rejected, single-field patch accepted, `listingPhotoUrl` + `useCanonicalPhoto:true` rejected, `listingPhotoUrl:null` + `useCanonicalPhoto:true` accepted; (b) **handler defense-in-depth** via `tool.execute()` (which bypasses schema parsing, matching the Task 1.5C test pattern) — `NO_PATCH_FIELDS` thrown without touching service or audit, `CONFLICTING_PHOTO_INPUTS` thrown without touching service or audit; (c) **happy path + errors** — service threading + audit shape with `patchedFields` array, `useCanonicalPhoto:true` with `listingPhotoUrl:null` keeps `useCanonicalPhoto` in `patchedFields`, `LISTING_NOT_FOUND`/`INVALID_STATUS_TRANSITION` translation skips audit, audit-failure isolation; (d) registry metadata — `readOnly:false`, no `needsApproval` on the built tool. Imports the exported `inputSchema` for direct validation.

**Modified:**
- `lib/thumper/tools/index.ts` — three new tool imports added (`searchJewelryDatabaseTool`, `updateListingTool`, `getTradeHistoryTool`); `REGISTRY` extended from 6 → 9 entries.
- `lib/thumper/system-prompt.ts` — full sweep of every tool-count and scope reference: line 57 `"six tools"` → `"nine tools"`; tool inventory bullets (3 new entries with descriptions matching the existing style, including the explicit "editable surface is exactly four fields" + "MSRP/design data NOT editable" guardrail in `update_listing` and the "approved + denied (not pending)" disambiguation in `get_trade_history`); new boundary bullet ("never call update_listing without a clear listingId; never call without patch fields"); Section 3 "your scope covers two areas" → "your scope covers four areas: managing the rep's board (list, add, edit, remove), handling incoming trade requests (view, approve, reject), reviewing past trades (history + analytics), and looking up pieces in the shared catalog"; Section 3 not-yet bullet rewritten from "Editing an existing listing's photo, description, price, trade preferences, or notes — Not yet" to "Editing a listing's MSRP, design name, material, main stone, or any other catalog/design metadata — Not yet, and probably never. The catalog is shared across reps; you can edit your own notes, trade preferences, and listing photo via update_listing, but the underlying design data is read-only from your seat" — narrower guardrail that explicitly carves out the four editable fields while keeping MSRP/catalog data off-limits; fallback decline template extended to enumerate all 9 tools' user-facing descriptions.
- `package.json` — `test` script extended to include `tests/thumper/trade-board-tools.test.ts` and `tests/thumper/jewelry-database.test.ts` (the script is an explicit allow-list, not a glob).
- `CODEBASE_SNAPSHOT.md` — header date + HEAD line updated; `app/api/thumper/route.ts` description updated `6 tools → 9 tools`; three new entries added under `lib/thumper/tools/`; this session block.

**Service-client decision:** `search_jewelry_database` uses **admin** client (the `activeListingsCount` aggregate crosses reps). `update_listing` and `get_trade_history` use **auth** client (both are rep-scoped — UPDATE on `trade_listings` is rep-scoped via RLS, and trade history reads only from the rep's own listings). This matches the per-function client requirements documented in `lib/services/jewelry-database.ts:7-15`, `lib/services/trade-board.ts:13-21`, and `lib/services/trade-requests.ts:7-15`.

**Photo-input precedence (update_listing):** The service applies `useCanonicalPhoto:true` with precedence and ignores `listingPhotoUrl` in that branch (`lib/services/trade-board.ts:468-474`). To prevent silent contradiction between input and audit, the schema rejects `listingPhotoUrl` (non-null) + `useCanonicalPhoto:true` simultaneously, and the handler re-checks before calling the service. The allowed exception (`listingPhotoUrl: null` + `useCanonicalPhoto: true`) is fine — both express "clear the rep photo" — and the test `inputSchema.safeParse({listingId, listingPhotoUrl: null, useCanonicalPhoto: true})` proves it accepts.

**Files NOT touched (per task guardrails):** All `lib/services/*.ts` files (service layer is the source of truth — must match repo casing/types/return shapes), existing tool handlers `add-listing.ts` / `list-my-trade-board.ts` / `remove-listing.ts` / `get-trade-requests.ts` / `approve-trade.ts` / `reject-trade.ts`, `lib/thumper/auth.ts`, `lib/thumper/persistence.ts`, `app/api/thumper/route.ts`, no new migrations, no UI changes.

**Verification (2026-04-30):**
- `npx tsc --noEmit` — diff against pre-edit baseline `.local/tsc-baseline.txt` is empty (`TSC_DIFF_CLEAN`). Only pre-existing `tests/thumper/attack-5-poisoned-rep-notes.test.ts` errors remain (unrelated, accepted baseline — same 5-error baseline as Task 1.5C/B).
- `npm test` — 48/48 pass across 5 files: `abort-modes.test.ts` (4) + `add-listing-recovery.test.ts` (12) + `trade-requests.test.ts` (17 = 13 existing + 4 new `get_trade_history` cases) + `trade-board-tools.test.ts` (13 new) + `jewelry-database.test.ts` (6 new).
- Registry shape — `REGISTRY.length` goes 6 → 9; `buildAllTools(ctx)` returns `{ list_my_trade_board, remove_listing, add_listing, get_trade_requests, approve_trade, reject_trade, search_jewelry_database, update_listing, get_trade_history }`; `needsApproval`-survives-wrapping assertion at `lib/thumper/tools/index.ts:50-52` continues to pass for the two write tools that set it (`remove_listing`, `approve_trade` — `update_listing` is reversible and does NOT set it).
- System-prompt sweep — Grep tool against `lib/thumper/system-prompt.ts`: `pattern: "six tools|6 tools"` → 0 matches; `pattern: "scope covers two areas"` → 0 matches; `pattern: "Editing an existing listing'?s photo, description, price"` → 0 matches; `pattern: "search_jewelry_database|update_listing|get_trade_history"` → 5+ matches; `pattern: "nine tools|four areas"` → 2 matches.
...
| `update_show` | false | auth | `update_show` (patchedFields) |
| `cancel_show` | **true** | auth | `cancel_show` (status hash) |
