...
const NIC_NAC_MODEL_POLICY_DEFINITIONS: Record<NicNacModelPolicyKey, NicNacModelPolicyDefinition> = {
  human_default: {
    key: "human_default",
    provider: "openai",
...
export const NIC_NAC_MODEL_POLICIES: Record<NicNacModelPolicyKey, NicNacModelPolicy> = Object.fromEntries(
...
export function getNicNacModelPolicy(key: NicNacModelPolicyKey): NicNacModelPolicy {
  ...
  return {
    key: definition.key,
    provider: definition.provider,
    modelId: process.env[definition.envVar]?.trim() || definition.defaultModelId,
    reasoning: definition.reasoning,
    purpose: definition.purpose,
  };
}
