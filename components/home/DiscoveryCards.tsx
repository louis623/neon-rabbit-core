    ...
    <div className="relative z-10 mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {discoveryCards.map((card) => {
        ...
        return (
          <Link
            key={card.title}
            className="group flex min-h-36 flex-col justify-between rounded-[var(--sparkle-radius-sm)] border border-[var(--sparkle-border)] bg-[rgba(255,254,253,0.82)] p-5 shadow-[var(--sparkle-shadow-sm)] transition hover:-translate-y-0.5 hover:border-[var(--sparkle-border-strong)] hover:bg-white hover:shadow-[var(--sparkle-shadow-md)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--sparkle-rose)]"
            href={card.href}
          >
            <div className="flex gap-4">
              <Icon aria-hidden="true" className="mt-1 size-10 shrink-0 text-[var(--sparkle-rose)]" strokeWidth={1.8} />
              <div>
                <h2 className="font-[var(--font-playfair)] text-xl font-semibold leading-tight text-[var(--sparkle-plum-deep)]">
                  {card.title}
                </h2>
                <p className="mt-2 text-sm leading-6 text-[var(--sparkle-ink-muted)]">{card.body}</p>
              </div>
            </div>
            <ArrowRight
              aria-hidden="true"
