...
import shellStyles from './DashboardPlaceholder.module.css'
    ...
    <div className={styles.stack}>
      <section className={styles.heroCard}>
        <div className={styles.heroHeader}>
          <div>
            <div className={shellStyles.cardTitle}>Trade Board</div>
            <div className={shellStyles.cardSubtitle}>
          ...
          <div className={styles.heroActions}>
            {onOpenCustomerBoardPreview ? (
              <button
                type="button"
                className={shellStyles.helperButton}
              ...
              <a
                className={shellStyles.helperLink}
            ...
            <span className={shellStyles.rosterTag}>Default landing section</span>
          </div>
        </div>
        {actionState.error ? (
          <div className={shellStyles.actionError}>{actionState.error}</div>
        ) : null}
        {actionState.helperMessage ? (
          <div className={shellStyles.helperMessage}>{actionState.helperMessage}</div>
        ) : null}
      </section>

      {swapApprovalDraft ? (
        <div
          className={shellStyles.imagePreviewMask}
          role="dialog"
          aria-modal="true"
        ...
        >
          <div
            className={shellStyles.imagePreviewDialog}
            onClick={(event) => event.stopPropagation()}
          >
            <button
              type="button"
              className={shellStyles.imagePreviewClose}
              onClick={() => setSwapApprovalDraft(null)}
              disabled={approvingSwap}
            >
              Close
            </button>
            <div className={shellStyles.walletSettingsTitle}>Approve trade</div>
            <p className={shellStyles.helperNote}>
            ...
            <label className={shellStyles.searchField}>
              <span className={shellStyles.searchLabel}>
                Revealed item number (optional)
              </span>
              <input
                type="text"
                className={`${shellStyles.searchInput} ph-no-capture`}
            ...
            {normalizedRevealedItemNumber.startsWith('RG') ? (
              <label className={shellStyles.searchField}>
                <span className={shellStyles.searchLabel}>Ring size</span>
                <input
                  type="text"
                  className={`${shellStyles.searchInput} ph-no-capture`}
      ...
      <section className={styles.summaryCard}>
        <div className={styles.sectionHeader}>
          <div>
            <div className={shellStyles.walletSettingsTitle}>Today&apos;s trade work</div>
            <div className={shellStyles.helperNote}>
              {tradeStatusReady
                ? tradeWorkCount > 0
      ...
      <section className={styles.sectionCard}>
        <div className={styles.sectionHeader}>
          <div>
            <div className={shellStyles.walletSettingsTitle}>Quick add</div>
            <div className={shellStyles.helperNote}>
        ...
        {tradeBoardState.status === 'ready' && boardSummary ? (
          <div className={styles.quickAddRow}>
            <label className={shellStyles.searchField}>
              <span className={shellStyles.searchLabel}>Quick add by item number</span>
              <input
                type="text"
                className={`${shellStyles.searchInput} ph-no-capture`}
          ...
          <div className={shellStyles.cardFill}>
            <div className={shellStyles.loadingLine} />
            <div className={shellStyles.loadingLineShort} />
          </div>
        )}
      </section>
      ...
      <section className={styles.sectionCard}>
        <div className={styles.sectionHeader}>
          <div>
            <div className={shellStyles.walletSettingsTitle}>Browse board</div>
            <div className={shellStyles.helperNote}>
              Search the active board first. Open filters only when you need to narrow it down.
            </div>
          </div>
          <span className={shellStyles.rosterTag}>
            {tradeBoardState.status === 'ready' && boardSummary
        ...
        {tradeBoardState.status === 'ready' && boardSummary ? (
          <>
            <label className={shellStyles.searchField}>
              <span className={shellStyles.searchLabel}>Search your active board</span>
              <input
                type="text"
                className={`${shellStyles.searchInput} ph-no-capture`}
            ...
            >
              ...
              <div className={styles.filterGrid}>
                <select
                  ...
                  className={shellStyles.boardInventorySelect}
                ...
                {hasActiveInventoryBrowse ? (
                  <button
                    type="button"
                    className={shellStyles.boardInventoryReset}
            ...
            {hasActiveInventoryBrowse ? (
              inventoryResults.length > 0 ? (
                <div
                  className={shellStyles.boardInventoryCarousel}
                  aria-label="Filtered active board pieces"
                >
                  <div className={shellStyles.boardInventoryCarouselHeader}>
                    <span className={shellStyles.helperNote}>{carousel.rangeLabel}</span>
                    <div className={shellStyles.boardInventoryArrowGroup}>
                      <button
                        type="button"
                        className={shellStyles.boardInventoryArrow}
                  ...
                  {isInventoryBrowseLoading ? (
                    <div className={shellStyles.helperNote}>Loading board pieces...</div>
                  ) : null}
                  <div className={shellStyles.boardInventoryCarouselGrid}>
                    {carousel.visibleItems.map((listing) => {
                      const photoUrl = getTradeListingPhotoUrl(listing)
                      const display = getTradeListingDisplayFields(listing)
                      return (
                        <div key={listing.id} className={shellStyles.boardInventoryPieceCard}>
                          <button
                            type="button"
                            className={shellStyles.boardInventoryMediaButton}
                            aria-label={`Open image preview for ${display.designName}`}
                            onClick={() => setPreviewListing(listing)}
                          >
                            <span className={shellStyles.boardInventoryMedia}>
                              {photoUrl ? (
                                <img
                                  className={shellStyles.tradePieceImage}
                                  src={photoUrl}
                                  alt={display.designName}
                                  loading="lazy"
                                />
                              ) : (
                                <span className={shellStyles.tradePieceFallback}>
                                  {display.typePrefix}
                                </span>
                              )}
                            </span>
                          </button>
                          <div className={shellStyles.boardInventoryPieceBody}>
                            <div className={shellStyles.customerName}>{display.designName}</div>
                            <div className={shellStyles.tradePieceMetaLine}>
                            ...
                            <div className={shellStyles.timelineItem}>
                              {formatTradeMoney(display.bpMsrp)}
                            </div>
                          </div>
                          <button
                            type="button"
                            className={shellStyles.boardInventoryRemoveButton}
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div className={shellStyles.imagePreviewFrame}>
                      {previewPhotoUrl ? (
                        <img
                          src={previewPhotoUrl}
                          alt={previewDisplay.designName}
                          className={shellStyles.imagePreviewImage}
                        />
                      ) : (
                        <div className={shellStyles.tradePieceFallback}>
                          {previewDisplay.typePrefix}
                        </div>
                      )}
                    </div>
                    <div className={shellStyles.walletSettingsTitle}>
      ...
      {tradeRequestsState.status === 'ready' && requests.length > 0 ? (
        <section className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            <div>
              <div className={shellStyles.walletSettingsTitle}>Request inbox</div>
              <div className={shellStyles.helperNote}>
                Review incoming trade requests and approve the right swaps.
              </div>
            </div>
            <span className={shellStyles.rosterTag}>{`${requests.length} pending`}</span>
          </div>
          <div className={shellStyles.tradeList}>
            {requests.map((request) => {
              ...
              return (
                <div key={request.id} className={shellStyles.tradeRow}>
                  <div className={shellStyles.tradeIdentity}>
                    <div className={shellStyles.customerName}>{request.customerName}</div>
                    <div className={shellStyles.customerDate}>
                      Wants {requestedItemLabel}
                    </div>
                    <div className={shellStyles.helperNote}>{request.customerDescription}</div>
                    {request.revealScreenshot ? (
                      <a
                        className={shellStyles.tradeScreenshotLink}
                        href={`/api/nic-nac/trade-requests/${request.id}/reveal-screenshot`}
                        target="_blank"
                        rel="noreferrer"
                      >
                        <img
                          className={shellStyles.tradeScreenshotThumb}
                          src={`/api/nic-nac/trade-requests/${request.id}/reveal-screenshot`}
                          alt={`Reveal screenshot from ${request.customerName}`}
                        />
                        <span>
                          <span className={shellStyles.tradeScreenshotTitle}>
                            Reveal screenshot
                          </span>
                          <span className={shellStyles.tradeScreenshotMeta}>
      ...
      {tradeSwapCleanupState.status === 'ready' && cleanupItems.length > 0 ? (
        <section className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            <div>
              <div className={shellStyles.walletSettingsTitle}>Swap cleanup</div>
              <div className={shellStyles.helperNote}>
            ...
            <span className={shellStyles.rosterTag}>{`${cleanupItems.length} to finish`}</span>
          </div>
          <div className={shellStyles.tradeList}>
            {cleanupItems.map((item) => (
              <div key={item.swapId} className={shellStyles.tradeRow}>
                <div className={shellStyles.tradeIdentity}>
                  <div className={shellStyles.customerName}>{item.customerName}</div>
                  <div className={shellStyles.customerDate}>
      ...
      {fulfillmentQueueState.status === 'ready' && queueItems.length > 0 ? (
        <section className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            <div>
              <div className={shellStyles.walletSettingsTitle}>Fulfillment queue</div>
              <div className={shellStyles.helperNote}>
                Keep approved swaps moving until they are fully closed out.
              </div>
            </div>
            <span className={shellStyles.rosterTag}>{`${queueItems.length} active swaps`}</span>
          </div>
          <div className={shellStyles.tradeList}>
            {queueItems.map((item) => {
              const nextStatus = getNextFulfillmentStatus(item.status)
              return (
                <div key={item.fulfillmentId} className={shellStyles.tradeRow}>
                  <div className={shellStyles.tradeIdentity}>
                    <div className={shellStyles.customerName}>{item.customerName}</div>
                    <div className={shellStyles.customerDate}>
                  ...
                  <div className={shellStyles.tradeMeta}>
                    <span className={shellStyles.statusBadgeWarning}>{item.status}</span>
                  </div>
                  <div className={shellStyles.actionRow}>
                    {nextStatus ? (
                      <button
