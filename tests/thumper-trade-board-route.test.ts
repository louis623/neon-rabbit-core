...
import {
  ...
  PATCH,
  POST,
} from '@/app/api/thumper/trade-board/route'
...
describe('trade board route', () => {
  ...
  it('returns the authenticated rep trade board summary', async () => {
    ...
    const response = await GET(
      new Request(
        'http://localhost/api/thumper/trade-board?status=available&type=RG&collection=Birthday&sortBy=listed_at&sortOrder=asc&limit=8',
      ),
    )
