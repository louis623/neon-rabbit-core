...
import { makeGetFulfillmentQueueTool } from '@/lib/thumper/tools/get-fulfillment-queue'
import { makeUpdateFulfillmentStatusTool } from '@/lib/thumper/tools/update-fulfillment-status'
...
describe('update_fulfillment_status', () => {
  ...
  it('translates invalid transitions into ThumperToolError without auditing', async () => {
...
describe('fulfillment registry and prompt wiring', () => {
  ...
  it('system prompt documents the fulfillment queue tools and follow-up flow', () => {
    ...
    expect(THUMPER_SYSTEM_PROMPT).toContain('Want to add the piece you got')
  })
})
