...
import { GET, POST } from '@/app/api/thumper/jewelry-library/route'
...
describe('jewelry library route', () => {
  ...
  it('searches the shared jewelry catalog for the authenticated rep', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/thumper/jewelry-library?query=aurora&limit=10'),
    )
  ...
  it('adds a searched design onto the rep board', async () => {
    ...
    const response = await POST(
      new Request('http://localhost/api/thumper/jewelry-library', {
        method: 'POST',
