...
describe('Sparkle Suite master brand system', () => {
  it('contains the required core brand docs with critical language', () => {
    ...
    expect(index).toContain('07-design-kit-audit-brief.md')
    ...
    expect(brandSpec).toContain('Dance Floor')
  ...
  it('contains the reusable template library', () => {
    ...
    expect(hooks).toContain('Dance Floor')
    ...
    expect(newsletter).toContain('Rep education')
    expect(newsletter).toContain('one newsletter brand')
    expect(newsletter).toContain('Dance Floor')
  ...
  it('contains a repo-local skill and daily-use index guidance for the brand system', () => {
    ...
    expect(skill).toContain('dpl_95Z57PuyJYJvvHabc2bjGCNzpZ8t')
    expect(skill).toContain('go ahead and polish this')
    expect(skill).toContain('letting Louis think he is testing production')
    ...
    expect(index).toContain('Use the Sparkle Suite master brand system to write 3 TikTok hooks for the Dance Floor.')
  ...
  it('logs the May 11 public-site lesson and design-kit audit requirement', () => {
    const lesson = read('docs/sparkle-suite/brand/06-public-site-incident-lesson.md')
    expect(lesson).toContain('May 11, 2026')
    expect(lesson).toContain('Do not treat public-facing design, copy, or branding changes as routine engineering cleanup.')
    expect(lesson).toContain('dpl_95Z57PuyJYJvvHabc2bjGCNzpZ8t')
    expect(lesson).toContain('If the user is testing production, say whether the test is:')
    expect(lesson).toContain('go ahead and polish this')

    const auditBrief = read('docs/sparkle-suite/brand/07-design-kit-audit-brief.md')
    expect(auditBrief).toContain('Louis suspects some design-kit work may have been based on a rejected public-site version.')
    expect(auditBrief).toContain('docs/drive-import/sparkle-suite/design/SS_DesignKit_Velvet.md')
