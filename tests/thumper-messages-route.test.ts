...
import { GET, POST } from '@/app/api/thumper/messages/route'
...
describe('messages route', () => {
  ...
  it('returns the authenticated rep message center payload', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/thumper/messages?limit=10&type=announcement'),
    )
  ...
  it('creates a backup support request message', async () => {
    ...
    const response = await POST(
      new Request('http://localhost/api/thumper/messages', {
        method: 'POST',
