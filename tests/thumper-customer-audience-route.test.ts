...
import { GET, POST } from '@/app/api/thumper/customer-audience/route'
import { AuthError } from '@/lib/thumper/auth'

describe('GET /api/thumper/customer-audience', () => {
  beforeEach(() => {
  ...
  it('returns the authenticated rep audience summary', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/thumper/customer-audience?limit=5&channel=sms'),
    )
  ...
  it('returns 401 when the rep is not signed in', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/thumper/customer-audience'),
    )
