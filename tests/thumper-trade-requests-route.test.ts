...
import { GET, POST } from '@/app/api/thumper/trade-requests/route'
...
describe('trade requests route', () => {
  ...
  it('returns pending requests for the authenticated rep', async () => {
    ...
    const response = await GET(
      new Request(
        'http://localhost/api/thumper/trade-requests?status=pending&limit=12',
      ),
    )
  ...
  it('approves a request through the fallback action', async () => {
    ...
    const response = await POST(
      new Request('http://localhost/api/thumper/trade-requests', {
        method: 'POST',
