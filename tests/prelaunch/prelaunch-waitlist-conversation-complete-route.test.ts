...
import { POST } from '@/app/api/control-center/intake/waitlist-conversation-complete/route'

describe('POST /api/control-center/intake/waitlist-conversation-complete', () => {
  beforeEach(() => {
  ...
  it('marks a scheduled meeting lead as conversation complete without provider actions', async () => {
    getAuthenticatedOperatorMock.mockResolvedValueOnce({
      repId: 'operator-1',
      rep: { email: 'louis@neonrabbit.net' },
    })
    singleMock.mockResolvedValueOnce({
      data: { id: 'waitlist-1', lead_status: 'conversation_complete' },
      error: null,
    })

    const response = await POST(
      new Request(
        'http://localhost/api/control-center/intake/waitlist-conversation-complete',
        {
          method: 'POST',
  ...
  it('redirects form submissions back to the conversation complete view', async () => {
    getAuthenticatedOperatorMock.mockResolvedValueOnce({
      repId: 'operator-1',
      rep: { email: 'louis@neonrabbit.net' },
    })
    ...
    form.set('returnTo', '/control-center/intake?waitlist=conversation_complete')
    ...
    expect(response.headers.get('location')).toBe(
      'http://localhost/control-center/intake?waitlist=conversation_complete',
    )
  })
  ...
  it('returns 409 when the lead is not currently meeting scheduled', async () => {
    getAuthenticatedOperatorMock.mockResolvedValueOnce({
      repId: 'operator-1',
      rep: { email: 'louis@neonrabbit.net' },
    })
    ...
    await expect(response.json()).resolves.toEqual({
      error: 'This lead is not ready to mark as conversation complete.',
    })
  })
