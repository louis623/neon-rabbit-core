...
import { POST } from '@/app/api/control-center/intake/waitlist-setup-profile-drafted/route'

describe('POST /api/control-center/intake/waitlist-setup-profile-drafted', () => {
  beforeEach(() => {
  ...
  it('marks a completed conversation lead as setup profile drafted without provider actions', async () => {
    getAuthenticatedOperatorMock.mockResolvedValueOnce({
      repId: 'operator-1',
      rep: { email: 'louis@neonrabbit.net' },
    })
    singleMock.mockResolvedValueOnce({
      data: { id: 'waitlist-1', lead_status: 'setup_profile_drafted' },
      error: null,
    })

    const response = await POST(
      new Request(
        'http://localhost/api/control-center/intake/waitlist-setup-profile-drafted',
        {
          method: 'POST',
  ...
  it('redirects form submissions back to the setup profile drafted view', async () => {
    getAuthenticatedOperatorMock.mockResolvedValueOnce({
      repId: 'operator-1',
      rep: { email: 'louis@neonrabbit.net' },
    })
    ...
    form.set('returnTo', '/control-center/intake?waitlist=setup_profile_drafted')
    ...
    expect(response.headers.get('location')).toBe(
      'http://localhost/control-center/intake?waitlist=setup_profile_drafted',
    )
  })
  ...
  it('returns 409 when the conversation is not currently complete', async () => {
    getAuthenticatedOperatorMock.mockResolvedValueOnce({
      repId: 'operator-1',
      rep: { email: 'louis@neonrabbit.net' },
    })
    ...
    await expect(response.json()).resolves.toEqual({
      error: 'This lead is not ready to mark as setup profile drafted.',
    })
  })
