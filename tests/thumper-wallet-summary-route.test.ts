...
import { GET } from '@/app/api/thumper/wallet-summary/route'
import { AuthError } from '@/lib/supabase/auth'

describe('GET /api/thumper/wallet-summary', () => {
  beforeEach(() => {
  ...
  it('returns the authenticated rep wallet summary', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/thumper/wallet-summary?limit=5'),
    )
  ...
  it('returns 400 for an invalid limit', async () => {
    const response = await GET(
      new Request('http://localhost/api/thumper/wallet-summary?limit=nope'),
    )
  ...
  it('returns 401 when the rep is not signed in', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/thumper/wallet-summary'),
    )
