...
- June 22, 2026 later: Finder Nic-Nac durable telemetry is implemented and deployed. The route now attempts fail-open persistence of conversations, messages, and run telemetry for mission redirects and model-backed streams. The telemetry migration was applied manually through the Supabase Dashboard to the `sparkle-finder-auth` project and directly verified for tables, RLS, read-own policies, service-role grants, triggers, indexes, and required columns. Finder deployment `dpl_1Q9mZ7WG4eNFWnzhbG7Loco3PbDU` is aliased at `https://sparkle-finder-dev.vercel.app`. The remaining telemetry proof gap is the deployed linked-runtime smoke: local Vercel env pull returns sensitive production values as empty strings, so the smoke cannot create/clean its temporary confirmed Finder user from this terminal without a service-role credential or a secured smoke-runner environment.
