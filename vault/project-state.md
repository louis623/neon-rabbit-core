...
- **Latest pushed implementation checkpoint:** `36668b6 fix: make Nic-Nac model provider OpenAI-only`
- **Latest local checkpoints:** Nic-Nac duplicate physical listing confirmation, Suite internal Finder rep-claim eligibility hardening, internal Finder intake wording cleanup, env/plan drift cleanup, core surface tool-policy refinement, strict Lab/model-cost guardrails, and the OpenAI-only Nic-Nac model provider are pushed and deployed to the stable demo alias; repo-local vault notes are being updated after the deploy.
- **Stable demo URL / Louis review target:** `https://sparkle-suite-demo.vercel.app`
- **Current stable demo target:** `https://sparkle-suite-4rpgzoala-louis-2849s-projects.vercel.app`
...
| Supabase | Active for Sparkle Suite referrals and Trade Board | `rep_referral_paid_months` is applied/verified; `trade_listings.ring_size` was applied through the Supabase Dashboard and verified on June 10, 2026 |
