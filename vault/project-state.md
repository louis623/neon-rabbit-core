...
- **Latest pushed checkpoint:** `0089964 feat: gate Sparkle Lab model synthesis`
- **Latest local checkpoints:** Sparkle Lab model synthesis harness is pushed and deployed to the stable demo alias; repo-local vault notes are being updated after the deploy.
- **Stable demo URL / Louis review target:** `https://sparkle-suite-demo.vercel.app`
- **Current stable demo target:** `https://sparkle-suite-ozj1u1i2d-louis-2849s-projects.vercel.app`
...
Current local implementation progress toward that architecture: Suite Nic-Nac now has OpenAI-first model policy/provider routing, model/product/surface/actor/cost telemetry, a shared product context contract, a Suite route surface-gated tool intent policy, reusable core persona/surface prompts, automatic bounded safe memory context from existing `rep_notes`, duplicate physical Trade Board listing guidance, and internal Sparkle Lab budget cap helpers. Suite also has a server-only `/api/internal/finder/rep-claim` validator for Finder to claim a rep from the Secret Rep ID Number using a server token, an applied Sparkle Lab schema for bounded runs/findings/artifacts, a read-only internal `/control-center/lab` page, a feature-flagged deterministic manual Sparkle Lab runner endpoint, an authenticated weekly Sparkle Lab cron route wired for Sunday overnight but disabled by default, and a default-off model synthesis harness that can create Lab report artifacts only when `SPARKLE_LAB_MODEL_SYNTHESIS_ENABLED=true`. Finder code has been audited read-only but not edited yet. Secret Rep ID rep-facing copy/UI has not been changed yet.
