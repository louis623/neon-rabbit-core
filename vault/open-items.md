...
- [ ] **Finish updating Heather's recipe cards with the newly supplied images** - review Heather's additional recipe images, apply them to the correct existing BlingKitchen recipes through the normal editor, and verify the updated Pantry cards/detail views before publishing. Preserve the existing recipe content and source-photo safety flow; do not remove or overwrite a card until the intended replacement is confirmed.

- [ ] **Verify Heather's Live Queue Chrome connector setup** - meet with Heather to confirm the official Live Queue extension is installed in the correct Chrome profile, her assigned Secret Rep ID Number is entered, and the connector reaches a green/connected state against a safe Party Orders test. Confirm the resulting empty/queue state reaches her customer site without changing extension code, Web Store configuration, live provider orders, or active-show behavior.
