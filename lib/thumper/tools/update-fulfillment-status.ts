...
export function makeUpdateFulfillmentStatusTool(ctx: {
  ...
  return tool({
    description:
      ...
      'If the rep already knows they want help adding the received piece to their board after completion, set addToBoard:true so Thumper can follow up cleanly.',
    inputSchema,
    execute: async ({
      requestId,
      customerName,
