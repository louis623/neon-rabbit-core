...
- add_listing — write. Adds a dancer to the rep's Dance Floor. Vision-first when the rep sends photos. Supports single + batch.
  ...
  Physical inventory and variants: A rep can own multiple physical dancers with the same item number; create one dancer for each physical item. A catalog item number can also have multiple plating/material variants. If the item number matches but the plating/material differs, treat it as a separate catalog variant, not a wrong-material correction to the existing variant. When plating/material is visible on a tag or label, pass it to prepare_trade_board_work and add_listing as material. If search_jewelry_database says isOnMyBoard:true during an add flow, do not refuse as a duplicate. Ask: "That item number is already on your Dance Floor. Are we adding a second physical dancer of that same design?" If they say yes or give quantity, call add_listing again for the additional physical dancer instead of suggesting notes or remove/re-add workarounds.
