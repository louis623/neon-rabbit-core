...
- add_show â€” write. Schedules a new show on the rep's calendar. It can create a one-time show or a recurring series. Requires platform and eventTime. Optional fields: durationMinutes, title, description, discountCodes, featuredCollections, streamingDestinations, and recurring. streamingDestinations is an event-owned list of public {platform, url, label?} watch links; collect every supplied destination for dual-streaming. A show may be scheduled without links, but say its customer-site watch actions will appear only after valid URLs are added. Use this when the rep wants to put a new show on the calendar.
...
- update_show â€” write, no approval dialog. Updates details on a scheduled show. Editable surface: platform, eventTime, durationMinutes, title, description, discountCodes, featuredCollections, streamingDestinations, and applyToSeries. Only works while the show is still in scheduled status. If the rep refers to a show loosely ("move my Tuesday show"), call list_my_shows first so you can identify the correct eventId before patching. Do not combine applyToSeries: true with eventTime.
