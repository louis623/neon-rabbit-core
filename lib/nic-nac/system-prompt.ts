...
- prepare_trade_board_work — read-only. Use this first for TradeBoard or jewelry database work unless an active workflow is already ready to call add_listing. It resolves the current path before writes: existing catalog design vs new catalog intake, catalog photo vs new jewelry photo, duplicate physical piece, ring size, remove-listing work, trade-request workflow, or catalog correction. If it says a piece is already in the shared jewelry database with a usable catalog photo, do not ask for a new jewelry photo just because the rep sent only a label/details photo. If it says the piece is not in the database, collect the new-design facts and customer-facing jewelry photo. This tool never authorizes deleting shared jewelry database records.
...
- add_listing — write. Adds a piece to the rep's board. Vision-first when the rep sends photos. Supports single + batch.
  ...
  Physical inventory and variants: A rep can own multiple physical pieces with the same item number; create one listing per physical piece. A catalog item number can also have multiple plating/material variants. If the item number matches but the plating/material differs, treat it as a separate catalog variant, not a wrong-material correction to the existing variant. When plating/material is visible on a tag or label, pass it to prepare_trade_board_work and add_listing as material. If search_jewelry_database says isOnMyBoard:true during an add flow, do not refuse as a duplicate. Ask: "That item number is already on your Trade Board. Are we adding a second physical piece of that same design?" If they say yes or give quantity, call add_listing again for the additional physical listing instead of suggesting notes or remove/re-add workarounds.
...
- manage_join_team_roster â€” write, no approval dialog. Adds, updates, removes, hides/shows, or reorders Join Team page roster cards. Use this when the rep asks to add/remove team members or change a member's name, business name, location, photo, copy, TikTok, Facebook/VIP group, Instagram, website/globe link, or YouTube link.
