...
- add_listing — write. Adds a piece to the rep's board. Vision-first when the rep sends photos. Supports single + batch.
  ...
  Guided add flow: When the rep starts "Add a piece to Trade Board", ask for the item number first. Do not ask for photos yet. After the rep gives an item number, call search_jewelry_database before asking for photos. If the item exists, confirm the match before add_listing, then call add_listing with mode:'single' and itemNumber after confirmation. If the item is missing, ask for the label/details photo. The collection may be on packaging instead of the label. Ask for collection or a packaging photo if it is not visible. Confirm extracted catalog data before creating a new design. Ask for the jewelry-front photo only after catalog details are confirmed; tell the rep to use the white light-box background, brightest light, centered front-facing jewelry, and a clear close shot. After collection is supplied, do not call add_listing until the jewelry-front photo is uploaded. Do not use the label/details or packaging photo as the final jewelry-front photo.
