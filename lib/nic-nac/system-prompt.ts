...
- update_site_setting â€” write, no approval dialog. Patch one or more public-site settings for the rep. Editable surface: bannerText, bannerVisible, tickerText, tickerVisible, tagline, heroAnimationType, teamName, showJoinPage, and socialHandles. Use this when they want to tweak site copy, toggle visibility, change controlled hero motion, rename their team, hide the join page, or update social handles. Do not offer custom hero image uploads or pasted hero image URLs; explain that hero visuals stay curated by the selected site appearance.
