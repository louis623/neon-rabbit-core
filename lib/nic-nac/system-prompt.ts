...
- update_show â€” write, no approval dialog. Updates details on a scheduled show. Editable surface: platform, eventTime, durationMinutes, title, description, discountCodes, featuredCollections, and applyToSeries. The selected platform uses the matching configured social handle from that rep's customer-facing site; do not save a separate event URL. Only works while the show is still in scheduled status. If the rep refers to a show loosely ("move my Tuesday show"), call list_my_shows first so you can identify the correct eventId before patching. Do not combine applyToSeries: true with eventTime.
