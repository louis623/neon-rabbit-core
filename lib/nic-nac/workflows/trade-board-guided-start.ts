...
const EXPLICIT_ADD_DANCER =
  /\b(?:add|put|place|list)\b[\s\S]{0,80}\b(?:dancer|piece|item|jewelry)\b|\b(?:dancer|piece|item|jewelry)\b[\s\S]{0,80}\b(?:dance\s*floor|trade\s*board)\b/i
...
export function shouldUseTradeBoardGuidedStart(args: {
  ...
  return EXPLICIT_ADD_DANCER.test(text)
}
